...
const template = require('ozaria/site/templates/play/level/tome/tome.jade');
...
module.exports = (TomeView = (function() {
  TomeView = class TomeView extends CocoView {
    ...
    onCommentMyCode(e) {
      for (let spellKey in this.spells) {
        ...
        if (spell.canWrite()) {
          ...
          const commentedSource = spell.view.commentOutMyCode() + 'Commented out to stop infinite loop.\n' + spell.getSource();
    ...
    createWorker() {
      ...
      if (window.application.isIPadApp) { return null; }  // Save memory!
      return new Worker('/javascripts/workers/aether_worker.js');
    }

    generateTeamSpellMap(spellObject) {
      const teamSpellMap = {};
      for (let spellName in spellObject) {
        const spell = spellObject[spellName];
        const teamName = spell.team;
        if (teamSpellMap[teamName] == null) { teamSpellMap[teamName] = []; }

        const spellNameElements = spellName.split('/');
        const thangName = spellNameElements[0];
    ...
    createSpells(programmableThangs, world) {
      ...
      for (thang of Array.from(programmableThangs)) {
        ...
        for (let methodName in thang.programmableMethods) {
          const method = thang.programmableMethods[methodName];
          const pathComponents = [thang.id, methodName];
          pathComponents[0] = _.string.slugify(pathComponents[0]);
          spellKey = pathComponents.join('/');
          this.thangSpells[thang.id].push(spellKey);
          const skipProtectAPI = utils.getQueryVariable('skip_protect_api', false);
      ...
      for (let thangID in this.thangSpells) {
        const spellKeys = this.thangSpells[thangID];
    ...
    onSpellLoaded(e) {
      for (let spellID in this.spells) {
    ...
    reloadAllCode() {
      ...
      for (let spellKey in this.spells) { const spell = this.spells[spellKey]; if (spell.view && ((spell.team === me.team) || (['common', 'neutral', null].includes(spell.team)))) { spell.view.reloadCode(false); } }
      return this.cast(false, false);
    }

    updateLanguageForAllSpells(e) {
      for (let spellKey in this.spells) { const spell = this.spells[spellKey]; if (spell.canWrite()) { spell.updateLanguageAether(e.language); } }
      if (e.reload) {
    ...
    destroy() {
      for (let spellKey in this.spells) { const spell = this.spells[spellKey]; spell.destroy(); }
      if (this.worker != null) {
