...
const template = require('templates/play/level/duel-stats-view');
  ...
  DuelStatsView = class DuelStatsView extends CocoView {
    ...
    constructor(options) {
      ...
      if (!options.otherSession) {
        options.otherSession = { get(prop) { return {
    ...
    update() {
      for (let player of Array.from(this.players)) {
        const thang = _.find(this.options.thangs, {id: this.avatars[player.team].thang.id});
    ...
    updatePower() {
      ...
      for (player of Array.from(this.players)) {
        const hero = _.find(this.options.thangs, {id: this.avatars[player.team].thang.id});
      ...
      for (let thang of Array.from(this.options.thangs)) {
    ...
    destroy() {
      ...
      for (let team in object) { const avatar = object[team]; avatar.destroy(); }
