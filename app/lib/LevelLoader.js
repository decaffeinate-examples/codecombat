...
module.exports = (LevelLoader = class LevelLoader extends CocoClass {
  ...
  loadSession() {
    ...
    if (this.sessionID) {
      ...
      if (this.spectateMode) { url += "?interpret=true"; }
    } else {
      let password;
    ...
    if (this.opponentSessionID) {
      const opponentURL = `/db/level.session/${this.opponentSessionID}?interpret=true`;
    ...
    if (this.session.loaded) {
      console.debug('LevelLoader: session already loaded:', this.session);
      this.session.setURL('/db/level.session/' + this.session.id);
      this.loadDependenciesForSession(this.session);
    } else {
      console.debug('LevelLoader: loading session:', this.session);
  ...
  loadDependenciesForSession(session) {
    ...
    if (this.level.isType('course') && (!me.showHeroAndInventoryModalsToStudents() || this.level.isAssessment())) {
      heroThangType = __guard__(me.get('heroConfig'), x => x.thangType) || ThangType.heroes.captain;
    ...
    let heroConfig = session.get('heroConfig');
    if ((session === this.session) && !this.headless) { if (heroConfig == null) { heroConfig = me.get('heroConfig'); } }
    ...
    for (let itemThangType of Array.from(_.values(heroConfig.inventory))) {
      var itemResource;
  ...
  loadCodeLanguagesForSession(session) {
    ...
    const resources = [];
    for (let codeLanguage of Array.from(codeLanguages)) {
  ...
  addSessionBrowserInfo(session) {
    ...
    if (!session.fake) { return session.patch(); }
  }
  ...
  populateLevel() {
    ...
    for (let thang of Array.from((this.level.get('thangs') || []).concat([flagThang]))) {
    ...
    for (let system of Array.from(this.level.get('systems') || [])) {
      var indieSprites;
      ...
      if (indieSprites = __guard__(system != null ? system.config : undefined, x => x.indieSprites)) {
        for (let indieSprite of Array.from(indieSprites)) {
    ...
    if (!this.headless || !!this.loadArticles) {
      for (let article of Array.from(__guard__(this.level.get('documentation'), x1 => x1.generalArticles) || [])) {
  ...
  loadThangsRequiredFromComponentList(components) {
    ...
    for (let component of Array.from(components)) {
      if (component.config) {
        if (component.original === LevelComponent.EquipsID) {
          for (let itemThangType of Array.from(_.values((component.config.inventory != null ? component.config.inventory : {})))) { requiredThangTypes.push(itemThangType); }
    ...
    return (() => {
      const result = [];
      for (let thangType of Array.from(extantRequiredThangTypes)) {
        const url = `/db/thang.type/${thangType}/version?project=name,components,original,rasterIcon,kind,prerenderedSpriteSheetData`;
  ...
  onThangNamesLoaded(thangNames) {
    for (let thangType of Array.from(thangNames.models)) {
  ...
  loadDefaultComponentsForThangType(thangType) {
    ...
    return (() => {
      const result = [];
      for (let component of Array.from(components)) {
        const url = `/db/level.component/${component.original}/version/${component.majorVersion}`;
  ...
  onWorldNecessityLoaded(resource) {
    ...
    this.worldNecessities = ((() => {
      const result = [];
      for (let r of Array.from(this.worldNecessities)) {         if (r != null) {
          result.push(r);
        }
      }
      return result;
...
//      @spriteSheetsToBuild.push res
  ...
  buildLoop() {
    ...
    for (let i = 0; i < iterable.length; i++) {
      const spriteSheetResource = iterable[i];
      ...
      if (thangType.loaded && !thangType.loading) {
        const keys = this.buildSpriteSheetsForThangType(spriteSheetResource.thangType);
        if (keys && keys.length) {
...
//      queue.loadFile('/file/'+f)
    ...
    const keys = [];
    for (let team of Array.from((left = this.thangTypeTeams[thangType.get('original')]) != null ? left : [null])) {
      var color;
      const spriteOptions = {resolutionFactor: SPRITE_RESOLUTION_FACTOR, async: true};
      ...
      const key = this.buildSpriteSheet(thangType, spriteOptions);
  ...
  grabThangTypeTeams() {
    ...
    this.thangTypeTeams = {};
    for (let thang of Array.from(this.level.get('thangs'))) {
  ...
  loadLevelSounds() {
    ...
    for (let script of Array.from(scripts)) {
      if (script.noteChain) {
        for (let noteGroup of Array.from(script.noteChain)) {
          if (noteGroup.sprites) {
            for (let sprite of Array.from(noteGroup.sprites)) {
      ...
      (() => {
        ...
        for (let trigger in object) {
          ...
          if (trigger !== 'say') {
            result.push((() => {
              const result1 = [];
              for (let sound of Array.from(sounds)) {                 result1.push(AudioPlayer.preloadSoundReference(sound));
              }
              return result1;
            })());
          }
