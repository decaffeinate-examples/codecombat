...
module.exports = (Angel = (function() {
  Angel = class Angel extends CocoClass {
    ...
    log() {
      ...
      for (let arg of Array.from(arguments)) { message += ` ${arg}`; }
      ...
      switch (event.data.type) {
        ...
        case 'non-user-code-problem':
          ...
          if (this.shared.firstWorld) {
            return this.infinitelyLooped(false, true);  // For now, this should do roughly the right thing if it happens during load.
          } else {
    ...
    finishBeholdingWorld(goalStates) { return world => {
      ...
      if (finished) {
        for (let scriptNote of Array.from(this.shared.world.scriptNotes)) {
    ...
    infinitelyLooped(escaped, nonUserCodeProblem) {
      if (escaped == null) { escaped = false; }
      if (nonUserCodeProblem == null) { nonUserCodeProblem = false; }
      this.say('On infinitely looped! Aborting?', this.aborting);
      if (this.aborting) { return; }
      const problem = {type: 'runtime', level: 'error', id: 'runtime_InfiniteLoop', message: 'Code never finished. It\'s either really slow or has an infinite loop.'};
      if (escaped) { problem.message = 'Escape pressed; code aborted.'; }
      this.publishGodEvent('user-code-problem', {problem});
      this.publishGodEvent('infinite-loop', {firstWorld: this.shared.firstWorld, nonUserCodeProblem});
    ...
    simulateSync(work) {
      ...
      if (this.shared.goalManager) {
        const goalManager = new GoalManager(work.world);
    ...
    simulateFramesSync(work, i) {
      ...
      while (i < world.totalFrames) {
        ...
        if (work.realTime) {
          ...
          const continuing = world.shouldContinueLoading(simulationLoopStartTime, (function() {}), false, (() => { if (!this.destroyed) { return this.simulateFramesSync(work); } }));
    ...
    finishSimulationSync(work) {
      ...
      for (let system of Array.from(work.world.systems)) { system.finish(work.world.thangs); }
      work.t2 = now();
