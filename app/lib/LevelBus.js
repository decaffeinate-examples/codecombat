...
module.exports = (LevelBus = (function() {
  LevelBus = class LevelBus extends Bus {
    ...
    onNewGoalStates(e) {
      ...
      let changed = false;
      for (let goalKey in newGoalStates) {
        const goalState = newGoalStates[goalKey];
        if (!me.isStudent()) {
    ...
    reallySaveSession() {
      ...
      const patch = {};
      for (let prop in this.changedSessionProperties) { patch[prop] = this.session.get(prop); }
      this.changedSessionProperties = {};
    ...
    destroy() {
      ...
      for (let destroyFunction of Array.from(this.vuexDestroyFunctions)) {
        destroyFunction();
      }
