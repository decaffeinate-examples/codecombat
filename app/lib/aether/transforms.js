...
module.exports.makeCheckThisKeywords = (makeCheckThisKeywords = (globals, varNames, language, problemContext) => (function(node) {
  if (node.type === S.VariableDeclarator) {
    ...
    return (() => {
      const result = [];
      for (let param of Array.from(node.params)) {           result.push(varNames[param.name] = true);
      }
      return result;
    })();
    ...
    if (v && !varNames[v] && !(Array.from(globals).includes(v))) {
      ...
      for (let p of Array.from(getParentsOfTypes(node, [S.FunctionDeclaration, S.FunctionExpression, S.VariableDeclarator, S.AssignmentExpression]))) {
        if (p.id != null) { varNames[p.id.name] = true; }
        if (p.left != null) { varNames[p.left.name] = true; }
        if (p.params != null) { for (let param of Array.from(p.params)) { varNames[param.name] = true; } }
