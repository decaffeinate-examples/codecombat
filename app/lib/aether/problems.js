...
var getTranspileHint = function(msg, context, languageID, code, range, simpleLoops) {
  ...
  if (["Unterminated string constant", "Unclosed string."].includes(msg) && (range != null)) {
    ...
    let parens = 0;
    for (let c of Array.from(lineStart)) { parens += (c === '(' ? 1 : c === ')' ? -1 : 0); }
...
class HintCreator {
  ...
  getScoreMatch(target, candidatesList) {
    ...
    for (let set of Array.from(candidatesList)) {
      if (set.candidates != null) {
        for (let match of Array.from(set.candidates)) {
          const matchScore = match.score(target, scoreFuzziness);
