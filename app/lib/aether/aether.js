...
module.exports = (Aether = (function() {
  Aether = class Aether {
    ...
    static deserialize(serialized) {
      ...
      for (let prop in serialized) { const val = serialized[prop]; if (prop !== "originalOptions") { aether[prop] = val; } }
      return aether;
    }
    ...
    transpile(raw) {
      ...
      if (/^\u56E7[a-zA-Z0-9+/=]+\f$/.test(rawCode)) {
        ...
        if (['cpp'].includes(this.language.id)) {
    ...
    lint(rawCode) {
      ...
      for (let problem of Array.from(this.language.lint(rawCode, this))) { this.addProblem(problem, lintProblems); }
      return lintProblems;
    }
    ...
    */
    purifyCode(rawCode) {
      ...
      const varNames = {};
      for (let parameter of Array.from(this.options.functionParameters)) { varNames[parameter] = true; }
