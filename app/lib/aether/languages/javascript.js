...
module.exports = (JavaScript = (function() {
  JavaScript = class JavaScript extends Language {
    ...
    lint(rawCode, aether) {
      ...
      const jshintOptions = {browser: false, couch: false, devel: false, dojo: false, jquery: false, mootools: false, node: false, nonstandard: false, phantom: false, prototypejs: false, rhino: false, worker: false, wsh: false, yui: false};
      ...
      for (let error of Array.from(jshintHolder.jshint.errors)) {
        ...
        for (let row = 0; row < lines.length; row++) {
          ...
          if (/^\s*if /.test(line)) {
            ...
            if ((firstParen = line.indexOf('(')) >= 0) {
              var i;
              let parenCount = 1;
              const iterable = line.slice(firstParen + 1, +line.length + 1 || undefined);
    ...
    parseDammit(code, aether) {
      const ast = acorn_loose.parse_dammit(code, {locations: true, tabSize: 4, ecmaVersion: 5});
