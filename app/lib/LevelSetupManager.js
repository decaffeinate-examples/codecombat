...
module.exports = (LevelSetupManager = class LevelSetupManager extends CocoClass {
  ...
  fillSessionWithDefaults() {
    ...
    const heroConfig = _.merge({}, me.get('heroConfig'), this.session.get('heroConfig'));
  ...
  loadModals() {
    ...
    if (this.level.isType('course-ladder', 'game-dev', 'web-dev') || (this.level.isType('course') && (!me.showHeroAndInventoryModalsToStudents() || this.level.isAssessment())) || window.serverConfig.picoCTF) {
      this.onInventoryModalPlayClicked();
      return;
    }
  ...
  open() {
    ...
    if (!_.isEqual(lastHeroesEarned, (left2 = __guard__(me.get('earned'), x2 => x2.heroes)) != null ? left2 : []) ||
      ...
      if (!_.contains(allowedHeroOriginals, __guard__(me.get('heroConfig'), x4 => x4.thangType))) {
        firstModal = this.heroesModal;
      }
    }


    lastHeroesEarned = (left4 = __guard__(me.get('earned'), x5 => x5.heroes)) != null ? left4 : [];
    lastHeroesPurchased = (left5 = __guard__(me.get('purchased'), x6 => x6.heroes)) != null ? left5 : [];
