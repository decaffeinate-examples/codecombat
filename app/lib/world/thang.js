  ...
  Thang = class Thang {
    ...
    static nextID(spriteName, world) {
      ...
      while (true) {
        ...
        const extantThang = world.thangMap[name];
    ...
    addComponents(...components) {
      ...
      return (() => {
        const result = [];
        for (let [componentClass, componentConfig] of Array.from(components)) {
          ...
          const c = new componentClass(componentConfig != null ? componentConfig : {});
          c.world = this.world;
    ...
    addTrackedProperties(...props) {
      ...
      return (() => {
        const result = [];
        for (let [prop, type] of Array.from(props)) {
          ...
          const oldPropIndex = this.trackedPropertiesKeys.indexOf(prop);
          if (oldPropIndex === -1) {
            ...
            const oldType = this.trackedPropertiesTypes[oldPropIndex];
            if (type !== oldType) {
    ...
    addTrackedFinalProperties(...props) {
      ...
      return this.trackedFinalProperties = this.trackedFinalProperties.concat(((() => {
        const result = [];
        for (let k of Array.from(props)) {           if (!(Array.from(this.trackedFinalProperties).includes(k))) {
            result.push(k);
          }
        }
        return result;
    ...
    callChainedMethod(methodName, ...args) {
      ...
      for (let componentMethod of Array.from(chain.components)) {
        const ret2 = componentMethod.apply(this, args);
    ...
    serialize() {
      ...
      for (let i = 0; i < iterable.length; i++) {
        ...
        const [componentClass, componentConfig] = iterable[i];
      ...
      for (let trackedFinalProperty of Array.from(this.trackedFinalProperties != null ? this.trackedFinalProperties : [])) {
      ...
      o.unusedTrackedPropertyKeys = ((() => {
        ...
        for (let propIndex = 0; propIndex < this.trackedPropertiesUsed.length; propIndex++) {
          const used = this.trackedPropertiesUsed[propIndex];
          if (!used) {
    ...
    static deserialize(o, world, classMap, levelComponents) {
      ...
      for (let [componentClassName, componentConfig] of Array.from(o.components)) {
        var componentClass;
        if (!(componentClass = classMap[componentClassName])) {
          ...
          const componentModel = _.find(levelComponents, {name: componentClassName});
    ...
    getLankOptions() {
      ...
      if (this.colors) {
        for (let colorType in this.colors) { const colorValue = this.colors[colorType]; options.colorConfig[colorType] = colorValue; }
      }
      return options;
    }
