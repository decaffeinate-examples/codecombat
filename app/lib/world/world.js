...
module.exports = (World = (function() {
  World = class World {
    ...
    destroy() {
      ...
      for (let thang of Array.from(this.thangs)) { thang.destroy(); }
      for (let key in this) { this[key] = undefined; }
      this.destroyed = true;
    ...
    thangDialogueSounds(startFrame) {
      ...
      for (let frameIndex = startFrame, end = this.frames.length, asc = startFrame <= end; asc ? frameIndex < end : frameIndex > end; asc ? frameIndex++ : frameIndex--) {
        ...
        for (let thangID in frame.thangStateMap) {
          var sayMessage;
          const state = frame.thangStateMap[thangID];
          if (!state.thang.say || !(sayMessage = state.getStateForProp('sayMessage'))) { continue; }
          const soundKey = state.thang.spriteName + ':' + sayMessage;
          if (!seen[soundKey]) {
    ...
    finishLoadingFrames(loadProgressCallback, loadedCallback, preloadedCallback) {
      if (!this.debugging) {
        this.ended = true;
        for (let system of Array.from(this.systems)) { system.finish(this.thangs); }
      }
      if (this.preloading) {
    ...
    adjustFlowSettings(loadUntilFrame) {
      return (() => {
        const result = [];
        for (let thang of Array.from(this.thangs)) {
          if (thang.isProgrammable) {
            var userCode = this.userCodeMap[thang.id] != null ? this.userCodeMap[thang.id] : {};
            result.push((() => {
              const result1 = [];
              for (let methodName in userCode) {
                const aether = userCode[methodName];
                const framesToLoadFlowBefore = (methodName === 'plan') || (methodName === 'makeBid') ? 200 : 1;  // Adjust if plan() is taking even longer
    ...
    loadFromLevel(level, willSimulate) {
      ...
      for (let system of Array.from(this.systems)) {
        try {
    ...
    loadSystemsFromLevel(level) {
      ...
      // Load new Systems
      for (let levelSystem of Array.from(level.systems)) {
        const systemModel = levelSystem.model;
        const {
          config
        } = levelSystem;
        const systemClass = this.loadClassFromCode(systemModel.js, systemModel.name, 'system');
    ...
    loadThangsFromLevel(level, willSimulate) {
      ...
      for (let thang of Array.from(toAdd)) { this.addThang(thang); }
      return null;
    }
    ...
    loadThangFromLevel(thangConfig, levelComponents, thangTypes, equipBy=null) {
      ...
      for (let componentIndex = 0; componentIndex < thangConfig.components.length; componentIndex++) {
        ...
        const componentModel = _.find(levelComponents, c => (c.original === component.original) && (c.version.major === (component.majorVersion != null ? component.majorVersion : 0)));
        const componentClass = this.loadClassFromCode(componentModel.js, componentModel.name, 'component');
    ...
    calculateBounds() {
      ...
      for (let thang of Array.from(this.thangs)) {  // Look at Lands only
        if (thang.isLand || (!hasLand && thang.rectangle)) {
          const rect = thang.rectangle().axisAlignedBoundingBox();
    ...
    publishNote(channel, event) {
      ...
      for (let script of Array.from(this.scripts != null ? this.scripts : [])) {
        if (script.channel !== channel) { continue; }
        const scriptNote = new WorldScriptNote(script, event);
    ...
    serialize() {
      ...
      for (let prop of Array.from(this.trackedProperties || [])) { o.trackedProperties[prop] = this[prop]; }

      for (let thangID in this.userCodeMap) {
        const methods = this.userCodeMap[thangID];
        const serializedMethods = (o.userCodeMap[thangID] = {});
      ...
      for (let thang of Array.from(this.thangs)) {
        ...
        const trackedPropertiesIndices = [];
        const trackedPropertiesKeys = [];
        const trackedPropertiesTypes = [];
        for (propIndex = 0; propIndex < thang.trackedPropertiesUsed.length; propIndex++) {
          const used = thang.trackedPropertiesUsed[propIndex];
          if (!used) { continue; }
      ...
      for (let thangIndex = 0; thangIndex < trackedPropertiesPerThangValues.length; thangIndex++) {
        const trackedPropertiesValues = trackedPropertiesPerThangValues[thangIndex];
        const trackedPropertiesValuesOffsets = o.trackedPropertiesPerThangValuesOffsets[thangIndex];
      ...
      for (i = 0; i < o.specialKeysToValues.length; i++) {
        const specialValue = o.specialKeysToValues[i];
      ...
      if (!typedArraySupport) {
        ...
        for (storage of Array.from(o.storageBuffer)) {
          for (let value of Array.from(storage)) {
    ...
    static deserialize(o, classMap, oldSerializedWorldFrames, finishedWorldCallback, startFrame, endFrame, level, streamingWorld) {
      ...
      if (streamingWorld) {
        ...
        for (thangID in o.userCodeMap) {
          const methods = o.userCodeMap[thangID];
          for (let methodName in methods) {
            const serializedAether = methods[methodName];
            for (let aetherStateKey of ['flow', 'metrics', 'style', 'problems']) {
      ...
      perf.t1 = now();
      if (w.thangs.length) {
        for (let thangConfig of Array.from(o.thangs)) {
      ...
      for (let thangIndex = 0; thangIndex < o.trackedPropertiesPerThangTypes.length; thangIndex++) {
        ...
        const trackedPropertyTypes = o.trackedPropertiesPerThangTypes[thangIndex];
        o.trackedPropertiesPerThangValues.push((trackedPropertiesValues = []));
        const trackedPropertiesValuesOffsets = o.trackedPropertiesPerThangValuesOffsets[thangIndex];
        for (let propIndex = 0; propIndex < trackedPropertyTypes.length; propIndex++) {
          const type = trackedPropertyTypes[propIndex];
          let storage = ThangState.createArrayForType(type, nFrames, o.storageBuffer, trackedPropertiesValuesOffsets[propIndex])[0];
          if (!typedArraySupport) {
    ...
    findFirstChangedFrame(oldWorld) {
      ...
      for (i = 0; i < this.frames.length; i++) {
        const newFrame = this.frames[i];
        const oldFrame = oldWorld.frames[i];
    ...
    actionsForThang(thangID, keepIdle) {
      ...
      for (let i = 0; i < states.length; i++) {
        ...
        const action = state != null ? state.getStateForProp('action') : undefined;
    ...
    getTeamColors() {
      ...
      const colorConfigs = {};
      for (let teamName in teamConfigs) { const config = teamConfigs[teamName]; colorConfigs[teamName] = config.color; }
      return colorConfigs;
    }

    teamForPlayer(n) {
      const playableTeams = this.playableTeams != null ? this.playableTeams : ['humans'];
      if (n != null) {
