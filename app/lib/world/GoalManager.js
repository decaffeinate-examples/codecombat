...
module.exports = (GoalManager = (function() {
  GoalManager = class GoalManager extends CocoClass {
    ...
    init() {
      ...
      if ((this.options != null ? this.options.session : undefined) && (this.options != null ? this.options.additionalGoals : undefined)) {
        const state = this.options.session.get('state');
        const {
          capstoneStage
        } = state;
        const stages = _.filter(this.options.additionalGoals, ag => ag.stage <= capstoneStage);
        const goals = _.map(stages, stage => stage.goals);
    ...
    updateGoalStates(newGoalStates) {
      for (let goalID in newGoalStates) {
        const goalState = newGoalStates[goalID];
    ...
    addAdditionalGoals(session, additionalGoals) {
      ...
      _.forEach(additionalGoals, stageGoals => {
        if (stageGoals.stage === capstoneStage) {
          return _.forEach(stageGoals.goals, goal => {
            if (!_.find(this.goals, existingGoal => goal.id === existingGoal.id)) {
              this.addGoal(goal);
              return goalsAdded = true;
            }
          });
        }
      });
      if (goalsAdded) {
        let left;
        const state = (left = session.get('state')) != null ? left : {};
    ...
    notifyGoalChanges() {
      ...
      const event = {
        ...
        goals: this.goals,
        overallStatus,
        timedOut: (this.world != null) && ((this.world.totalFrames === this.world.maxTotalFrames) && !['success', 'failure'].includes(overallStatus))
      };
    ...
    initGoalStates() {
      ...
      return (() => {
        const result = [];
        for (let goal of Array.from(this.goals)) {
          const state = {
          ...
          for (let getTo of Array.from(goal.getAllToLocations != null ? goal.getAllToLocations : [])) {
            this.initGoalState(state, [(getTo.getToLocation != null ? getTo.getToLocation.who : undefined), []], 'arrived');
          }
          for (let keepFrom of Array.from(goal.keepAllFromLocations != null ? goal.keepAllFromLocations : [])) {
    ...
    checkForInitialUserCodeProblems() {
      ...
      return Array.from(this.world.thangs).filter((thang) => thang.isProgrammable).map((thang) =>
        (() => {
          const result = [];
          for (let message in thang.publishedUserCodeProblems) {
            const problem = thang.publishedUserCodeProblems[message];
    ...
    onThangDied(e, frameNumber) {
      return (() => {
        const result = [];
        for (let goal of Array.from(this.goals != null ? this.goals : [])) {
      ...
      return (() => {
        ...
        for (var goal of Array.from(this.goals != null ? this.goals : [])) {
          ...
          if (goal.getAllToLocations != null) {
            for (let getTo of Array.from(goal.getAllToLocations)) {
    ...
    wrapUpGoalStates(finalFrame) {
      return (() => {
        const result = [];
        for (let goalID in this.goalStates) {
      ...
      if (overallStatus = this.checkOverallStatus(true)) {
        const matchedGoals = ((() => {
          ...
          for (goalID in this.goalStates) {
            const goalState = this.goalStates[goalID];
    ...
    goalIsPositive(goalID) {
      ...
      for (let prop in goal) { if (this.positiveGoalMap[prop] === 0) { return false; } }
      return true;
    }
