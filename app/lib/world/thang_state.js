  ...
  ThangState = class ThangState {
    ...
    constructor(thang) {
      ...
      for (let propIndex = 0; propIndex < thang.trackedPropertiesKeys.length; propIndex++) {
        const prop = thang.trackedPropertiesKeys[propIndex];
        const type = thang.trackedPropertiesTypes[propIndex];
        const value = thang[prop];
        if (type === 'Vector') {
    ...
    restore() {
      ...
      if (!this.hasRestored) {  // Restoring in a deserialized World for first time
        ...
        for (propIndex = 0; propIndex < this.trackedPropertyKeys.length; propIndex++) {
          ...
          const type = this.trackedPropertyTypes[propIndex];
          const storage = this.trackedPropertyValues[propIndex];
    ...
    restorePartial(ratio) {
      ...
      for (let propIndex = 0; propIndex < this.trackedPropertyKeys.length; propIndex++) {
        const prop = this.trackedPropertyKeys[propIndex];
    ...
    serialize(frameIndex, trackedPropertyIndices, trackedPropertyTypes, trackedPropertyValues, specialValuesToKeys, specialKeysToValues) {
      ...
      for (let newPropIndex = 0; newPropIndex < trackedPropertyTypes.length; newPropIndex++) {
        const type = trackedPropertyTypes[newPropIndex];
        const originalPropIndex = trackedPropertyIndices[newPropIndex];
        const storage = trackedPropertyValues[newPropIndex];
        let value = this.props[originalPropIndex];
        if (value) {
          ...
          if (type === 'Vector') {
            ...
            const stringPieces = ['\x1D'];  // Group Separator
