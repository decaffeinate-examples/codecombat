...
module.exports = (ScriptManager = (ScriptManager = (function() {
  ScriptManager = class ScriptManager extends CocoClass {
    ...
    addScriptSubscriptions() {
      ...
      return (() => {
        const result = [];
        for (let script of Array.from(this.scripts)) {
          if (!script.id) { script.id = (idNum++).toString(); }
          const callback = makeCallback(script.channel); // curry in the channel argument
    ...
    addPartiallyEndedScriptFromSession() {
      ...
      if (scripts.currentScriptOffset) {
        for (let noteGroup of Array.from(noteChain.slice(0, +(scripts.currentScriptOffset-1) + 1 || undefined))) { noteGroup.skipMe = true; }
      }
    ...
    addEndedScriptsFromSession() {
      ...
      for (let scriptID of Array.from(scriptsToSkip)) {
        const script = _.find(this.scripts, {id: scriptID});
        if (!script) {
        ...
        const noteChain = this.processScript(script);
        if (!noteChain) { return; }
        for (let noteGroup of Array.from(noteChain)) { noteGroup.skipMe = true; }
    ...
    initializeCamera() {
      ...
      for (let script of Array.from(this.scripts)) {
        for (let note of Array.from(script.noteChain || [])) {
          if ((note.surface != null ? note.surface.focus : undefined) != null) {
            const surfaceModule = _.find(note.modules || [], module => module.surfaceCameraNote);
            if (surfaceModule) {
              const cameraNote = surfaceModule.surfaceCameraNote(true);
    ...
    onNote(channel, event) {
      ...
      for (let script of Array.from(this.scripts)) {
        const alreadyTriggered = Array.from(this.triggered).includes(script.id);
        ...
        if (script.notAfter) {
          for (let scriptID of Array.from(script.notAfter)) {
    ...
    processScript(script) {
      ...
      for (let noteGroup of Array.from(noteChain)) { noteGroup.scriptID = script.id; }
    ...
    processNoteGroup(noteGroup) {
      ...
      return noteGroup.modules = ((() => {
        const result = [];
        for (let Module of Array.from(allScriptModules)) {           if (Module.neededFor(noteGroup)) {
    ...
    run() {
      ...
      for (let module of Array.from(nextNoteGroup.modules)) {
        for (let note of Array.from(module.startNotes())) { this.processNote(note, nextNoteGroup); }
      }
    ...
    skipAhead() {
      ...
      for (i = 0; i < this.noteGroupQueue.length; i++) {
        const noteGroup = this.noteGroupQueue[i];
        if (!noteGroup.skipMe) { break; }
        if (this.debugScripts) { console.debug(`SCRIPT: Skipping note group '${noteGroup.name}'`); }
        this.processNoteGroup(noteGroup);
        for (let module of Array.from(noteGroup.modules)) {
          const notes = module.skipNotes();
          for (let note of Array.from(notes)) { this.processNote(note, noteGroup); }
        }
    ...
    endNoteGroup() {
      ...
      for (let timeout of Array.from(this.currentTimeouts)) { clearTimeout(timeout); }
      for (let module of Array.from(this.currentNoteGroup.modules)) {
        for (let note of Array.from(module.endNotes())) { this.processNote(note, this.currentNoteGroup); }
      }
    ...
    endAll(options) {
      ...
      for (let i = 0; i < this.noteGroupQueue.length; i++) {
        ...
        for (let module of Array.from(noteGroup.modules)) {
          const notes = module.skipNotes();
          if (!this.quiet) { for (let note of Array.from(notes)) { this.processNote(note, noteGroup); } }
        }
