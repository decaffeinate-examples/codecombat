...
class Agent {
  ...
  waitForCodeCombatLoaded() {
    return new Promise((res, rej) => {
      ...
      return this.iframe.contentWindow.application.router.once('did-load-route', () => //TODO: Wait for supermodel to be loaded.
      res());
    });
  }
  ...
  clickAndWaitForRoute(what) {
    return this.findAndWait(what).then(target => {
      return new Promise((res, rej) => {
        target.click();
        this.iframe.contentWindow.application.router.once('did-load-route', () => //TODO: Wait for supermodel to be loaded.
        res());
        return target.click();
      });
    });
      ...
      this.prototype.tests = {
        homepageLoad: (agent, log) => {
          ...
          .then(() => agent.retreiveTimings()).then(data => {
            ...
            return (() => {
              const result = [];
              for (let k in data.timing) {
                const delta = data.timing[k] - data.timing.navigationStart;
