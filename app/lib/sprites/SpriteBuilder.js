...
module.exports = (SpriteBuilder = class SpriteBuilder {
  ...
  buildMovieClip(animationName, mode, startPosition, loops, labels) {
    ...
    for (let tweenData of Array.from(animData.tweens)) {
      let tween = createjs.Tween;
      let stopped = false;
      for (let func of Array.from(tweenData)) {
        const args = _.cloneDeep(func.a);
  ...
  buildMovieClipContainers(localContainers) {
    ...
    for (let localContainer of Array.from(localContainers)) {
      const container = this.buildContainerFromStore(localContainer.gn);
  ...
  buildMovieClipGraphics(localGraphics) {
    const map = {};
    for (let localGraphic of Array.from(localGraphics)) {
      const graphic = new createjs.Graphics().p(localGraphic.p);
  ...
  buildContainerFromStore(containerKey) {
    ...
    cont.initialize();
    for (let childData of Array.from(contData.c)) {
      var child;
...
//    colorConfig ?= {team: {hue:0.4, saturation: -0.5, lightness: -0.5}} # test config
    ...
    return (() => {
      const result = [];
      for (let group in colorConfig) {
        const config = colorConfig[group];
  ...
  buildOzariaColorMapForGroup(shapes, config) {
    ...
    return (() => {
      const result = [];
      for (let shapeKey of Array.from(shapes)) {
        const shape = this.shapeStore[shapeKey];
  ...
  initColorMap(shapes) {
    ...
    for (let shapeKey of Array.from(shapes)) {
      const shape = this.shapeStore[shapeKey];
      if (((shape.fc == null)) || colors[shape.fc]) { continue; }
      const hsl = hexToHSL(shape.fc);
      colors[shape.fc] = hsl;
    }
    return colors;
  }
