...
module.exports = (SpriteParser = class SpriteParser {
  ...
  populateLongKeys() {
    ...
    return (() => {
      ...
      for (shortKey in this.thangType.animations) {
        const animation = this.thangType.animations[shortKey];
  ...
  parse(source) {
    ...
    for (index = 0; index < containers.length; index++) {
      ...
      const addChildArgs = this.getAddChildCallArguments(container, source);
      const instructions = [];
      for (let bn of Array.from(addChildArgs)) {
        let gotIt = false;
        for (let shape of Array.from(localShapes)) {
          if (shape.bn === bn) {
    ...
    for (index = 0; index < movieClips.length; index++) {
      ...
      for (let boundsIndex = 0; boundsIndex < movieClip.frameBounds.length; boundsIndex++) {
      ...
      const localGraphics = this.getGraphicsFromBlock(movieClip, source);
      ...
      const localAnimations = this.getAnimationsFromMovieClip(movieClip, source, true);
      for (let animation of Array.from(localAnimations)) {
        childrenMovieClips.push(animation.gn);
      }
      const localTweens = this.getTweensFromMovieClip(movieClip, source, localShapes, localContainers, localAnimations);
      this.addAnimation({
        shapes: localShapes,
          ...
          if (frameBoundsStatement) {
            ...
            if (frameBoundsSource.search(/\[rect/) === -1) {  // some other statement; we don't have multiframe bounds
              ...
              for (let i = 0; i < frameBoundsStatement.expression.right.elements.length; i++) {
                const arg = frameBoundsStatement.expression.right.elements[i];
                bounds = null;
                const argSource = this.subSourceFromRange(arg.range, source);
  ...
  */
  ...
  getShapesFromBlock(block, source) {
    ...
    const gatherShapeDefinitions = function(node) {
      ...
      for (let statement of Array.from(body.slice(graphicsStatementIndex + 2))) {
        // Handle things like
