...
module.exports = (God = (function() {
  God = class God extends CocoClass {
    ...
    destroy() {
      for (let angel of Array.from(this.angelsShare.angels.slice())) { angel.destroy(); }
    ...
    createWorld({spells, preload, realTime, justBegin, keyValueDb, synchronous}) {
      ...
      const userCodeMap = this.getUserCodeMap(spells);
      ...
      for (angel of Array.from(this.angelsShare.busyAngels.slice())) {
        const isPreloading = angel.running && angel.work.preload && _.isEqual(angel.work.userCodeMap, userCodeMap, function(a, b) {
    ...
    filterUserCodeMapWhenFromWorld(worldUserCodeMap) {
      ...
      for (let thangName in worldUserCodeMap) {
        const thang = worldUserCodeMap[thangName];
        newUserCodeMap[thangName] = {};
        for (let spellName in thang) {
          const aether = thang[spellName];
          const shallowFilteredObject = _.pick(aether, ['raw', 'pure', 'originalOptions']);
