...
module.exports = (Lank = (Lank = (function() {
  Lank = class Lank extends CocoClass {
    ...
    onThangTypeLoaded() {
      ...
      if (this.options.preloadSounds) {
        ...
        for (let trigger in object) {
          const sounds = object[trigger];
          if (trigger !== 'say') {
            for (let sound of Array.from(sounds)) { if (sound) { AudioPlayer.preloadSoundReference(sound); } }
          }
        }
      }
    ...
    setSprite(newSprite) {
      ...
      for (let prop of ['lastPos', 'currentRootAction']) {
        delete this[prop];
      }
    ...
    stop() {
      ...
      for (let name in this.marks) { const mark = this.marks[name]; mark.stop(); }
      return this.stopped = true;
    }

    play() {
      __guardMethod__(this.sprite, 'play', o => o.play());
      for (let name in this.marks) { const mark = this.marks[name]; mark.play(); }
    ...
    showAreaOfEffects() {
      ...
      return (() => {
        ...
        for (var event of Array.from(this.thang.currentEvents)) {
          ...
          const args = JSON.parse(event.slice(4));
          const key = 'aoe-' + JSON.stringify(args.slice(2));
          const layerName = args[6] != null ? args[6] : 'ground';  // Can also specify 'floating'.
          ...
          if (!Array.from(layer.spriteSheet.animations).includes(key)) {
            ...
            const radius = args[2] * Camera.PPM;
            if (args.length === 4) {
              circle.graphics.beginFill(args[3]).drawCircle(0, 0, radius);
            } else {
              let startAngle = args[4] || 0;
              let endAngle = args[5] || (2 * Math.PI);
          ...
          const pos = this.options.camera.worldToSurface({x: args[0], y: args[1]});
          circle.x = pos.x;
          circle.y = pos.y;
          const resFactor = layer.resolutionFactor;
    ...
    showTextEvents() {
      ...
      return (() => {
        const result = [];
        for (let event of Array.from(this.thang.currentEvents)) {
          var left;
          if (!_.string.startsWith(event, 'text-')) { continue; }
          if (this.handledDisplayEvents[event]) { continue; }
          this.handledDisplayEvents[event] = true;
          const options = JSON.parse(event.slice(5));
          var label = new createjs.Text(options.text, `bold ${options.size || 16}px Arial`, options.color || '#FFF');
          const shadowColor = (left = {humans: '#F00', ogres: '#00F', neutral: '#0F0', common: '#0F0'}[this.thang.team]) != null ? left : '#000';
  ...
  //    console.error 'No thang for', @ unless @thang
      ...
      const newScaleFactorX = (left2 = (this.thang != null ? this.thang.scaleFactorX : undefined) != null ? (this.thang != null ? this.thang.scaleFactorX : undefined) : (this.thang != null ? this.thang.scaleFactor : undefined)) != null ? left2 : 1;
      const newScaleFactorY = (left3 = (this.thang != null ? this.thang.scaleFactorY : undefined) != null ? (this.thang != null ? this.thang.scaleFactorY : undefined) : (this.thang != null ? this.thang.scaleFactor : undefined)) != null ? left3 : 1;
      if (((this.layer != null ? this.layer.name : undefined) === 'Land') || (this.thang != null ? this.thang.isLand : undefined) || ((this.thang != null ? this.thang.spriteName : undefined) === 'Beam') || this.isCinematicLank) {
    ...
    updateAlpha() {
      ...
      for (let name in this.marks) { const mark = this.marks[name]; mark.updateAlpha(this.thang.alpha); }
    ...
    determineAction() {
      ...
      if (this.actions[action] == null) {
        ...
        if (!this.warnedFor[action]) { console.warn('Cannot show action', action, 'for', this.thangType.get('name'), 'because it DNE'); }
    ...
    getActionDirection(rootAction=null) {
      ...
      if (relatedActions['111111111111']) {  // has grid-surrounding-wall-based actions
        if (this.wallGrid) {
          ...
          for (let y of [gy + tileSize, gy, gy - tileSize, gy - (tileSize * 2)]) {
            for (let x of [gx - tileSize, gx, gx + tileSize]) {
              var wallThangs;
          ...
          for (let relatedAction in relatedActions) {
    ...
    getActionProp(prop, subProp, def=null) {
      ...
      for (let val of [(this.currentAction != null ? this.currentAction[prop] : undefined), this.thangType.get(prop)]) {
  ...
  //      @addMark('bounds').toggle true if @thang?.drawsBounds
    ...
    updateMarks() {
      ...
      for (let name in this.marks) { const mark = this.marks[name]; mark.update(); }
    ...
    rotateEffect() {
      ...
      for (let effect of Array.from(effects)) { this.marks[effect].hide(); }
    ...
    setHighlight(to, delay) {
      ...
      return (this.marks.highlight != null ? this.marks.highlight.toggle(to && !this.dimmed) : undefined);
    }
    ...
    addLabel(name, style, labelOptions) {
      ...
      if (this.labels[name] == null) { this.labels[name] = new Label({sprite: this, camera: this.options.camera, layer: this.options.textLayer, style, labelOptions}); }
    ...
    playSounds(withDelay, volume) {
      ...
      for (let event of Array.from(this.thang.currentEvents != null ? this.thang.currentEvents : [])) {
    ...
    playSound(sound, withDelay, volume) {
      ...
      if (volume == null) { volume = 1.0; }
      if (_.isString(sound)) {
        const soundTriggers = utils.i18n(this.thangType.attributes, 'soundTriggers');
      ...
      const delay = withDelay && sound.delay ? (1000 * sound.delay) / createjs.Ticker.framerate : 0;
    ...
    destroy() {
      let name, p;
      for (name in this.marks) { const mark = this.marks[name]; mark.destroy(); }
      for (name in this.labels) { const label = this.labels[name]; label.destroy(); }
