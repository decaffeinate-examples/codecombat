...
module.exports = (LankBoss = (function() {
  LankBoss = class LankBoss extends CocoClass {
    ...
    destroy() {
      for (let thangID in this.lanks) { const lank = this.lanks[thangID]; this.removeLank(lank); }
      ...
      for (let lankLayer of Array.from(_.values(this.layerAdapters))) { lankLayer.destroy(); }
    ...
    onHighlightSprites(e) {
      ...
      return (() => {
        const result = [];
        for (let thangID in this.lanks) {
          const lank = this.lanks[thangID];
    ...
    update(frameChanged) {
      ...
      for (let lank of Array.from(this.lankArray)) { lank.update(frameChanged); }
    ...
    adjustLankExistence() {
      ...
      for (let thang of Array.from(this.world.thangs)) {
      ...
      for (let item of Array.from(itemsJustEquipped)) { item.modifyStats(); }
      for (let thangID in this.lanks) {
        lank = this.lanks[thangID];
        const missing = !(lank.notOfThisWorld || (this.world.thangMap[thangID] != null ? this.world.thangMap[thangID].exists : undefined));
        const isObstacle = lank.sprite.parent === this.layerAdapters['Obstacle'];
    ...
    updateScreenReader() {
      ...
      const grid = new Grid(thangs, this.world.width, this.world.height, 0, 0, 0, true);
      utils.replaceText(ascii, grid.toString(true));
      ascii.css('transform', 'initial');
      const fullWidth = ascii.innerWidth();
      const fullHeight = ascii.innerHeight();
      const availableWidth = ascii.parent().innerWidth();
      const availableHeight = ascii.parent().innerHeight();
      let scale = availableWidth / fullWidth;
    ...
    equipNewItems(thang) {
      ...
      if (thang.inventoryIDs) {
        ...
        for (let slot in thang.inventoryIDs) {
          const itemID = thang.inventoryIDs[slot];
          const item = this.world.getThangByID(itemID);
          if (!item.equipped) {
  ...
  //    console.log 'updating up to', possiblyUpdatedWallLanks.length, 'of', wallLanks.length, 'wall lanks from updatedObstacles', updatedObstacles
      for (let wallLank of Array.from(possiblyUpdatedWallLanks)) {
  ...
  //    console.log wallGrid.toString()
    ...
    onNewWorld(e) {
      ...
      if (e.finished && /kithgard-mastery/.test(window.location.href)) { return this.cachedObstacles = false; }
    }

    play() {
      for (let lank of Array.from(this.lankArray)) { lank.play(); }
      if (this.selectionMark != null) {
    ...
    stop() {
      for (let lank of Array.from(this.lankArray)) { lank.stop(); }
      if (this.selectionMark != null) {
    ...
    onStageMouseDown(e) {
      if (!this.handleEvents) { return; }
      if (key.shift) { return; } //and @options.choosing
      if (e.onBackground) { return this.selectLank(e); }
    }
    ...
    onChangeSelected(gameUIState, selected) {
      ...
      for (lank of Array.from(addedLanks)) {
        const layer = lank.sprite.parent !== this.layerAdapters.Default.container ? this.layerAdapters.Default : this.layerAdapters.Ground;
        const mark = new Mark({name: 'selection', camera: this.camera, layer, thangType: 'selection'});
        mark.toggle(true);
        mark.setLank(lank);
        mark.update();
    ...
    onFlagColorSelected(e) {
      ...
      for (let flagLank of Array.from(this.lankArray)) {
    ...
    onFlagAppeared(e) {
      ...
      for (let i = pending.length - 1; i >= 0; i--) {
        const pendingFlag = pending[i];
        const t2 = pendingFlag.thang;
        const matchedType = (t1.color === t2.color) && (t1.team === t2.team);
        const matched = matchedType && (foundExactMatch || ((Math.abs(t1.pos.x - t2.pos.x) < 0.00001) && (Math.abs(t1.pos.y - t2.pos.y) < 0.00001)));
        if (matched) {
