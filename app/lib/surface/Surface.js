...
module.exports = (Surface = (Surface = (function() {
  Surface = class Surface extends CocoClass {
    ...
    constructor(world, normalCanvas, webGLCanvas, givenOptions) {
      ...
      this.zoomToHero = this.options.levelType !== "game-dev"; // In game-dev levels the hero is gameReferee
  ...
  //    @cameraBorderLayer.addChild @cameraBorder = new CameraBorder(bounds: @camera.bounds)
    ...
    initCoordinates() {
      ...
      if (this.showCoordinates) { return this.coordinateDisplay != null ? this.coordinateDisplay : (this.coordinateDisplay = new CoordinateDisplay({camera: this.camera, layer: this.surfaceTextLayer})); }
    }
    ...
    tick(e) {
      ...
      while (true) {
        ...
        if (frameAdvanced && this.playing) {
          let advanceBy = this.world.frameRate / this.options.frameRate;
        ...
        const newWorldFrame = Math.floor(this.currentFrame);
        if (Dropper.drop()) {
          ++framesDropped;
        } else {
    ...
    restoreScores(frame) {
      ...
      const scores = [];
      for (let scoreType of Array.from((left = this.options.level.get('scoreTypes')) != null ? left : [])) {
        var score;
    ...
    setProgress(progress, scrubDuration) {
      ...
      const onTweenEnd = () => {
        ...
        return this.scrubbingPlaybackSpeed = null;
      };
    ...
    onFramesScrubbed(e) {
      ...
      if (e) {
        ...
        while (true) {  // temporary fix to stop cacophony
          ...
          const frame = this.world.getFrame(this.getCurrentFrame());
          frame.restoreState();
          const volume = Math.max(0.05, Math.min(1, 1 / this.scrubbingPlaybackSpeed));
          for (let lank of Array.from(this.lankBoss.lankArray)) { lank.playSounds(false, volume); }
  ...
  //    @cameraBorder.updateBounds @camera.bounds
    ...
    onStreamingWorldUpdated(event) {
      ...
      if (this.playing && !this.realTime && (ffToFrame = Math.min(event.firstChangedFrame, this.frameBeforeCast, this.world.frames.length - 1)) && (ffToFrame > (this.currentFrame + (fastForwardBuffer * this.world.frameRate)))) {
        ...
        if (this.cinematic) {
          this.fastForwardingSpeed = Math.max(1, Math.min(2, (ffToFrame * this.world.dt) / 15));
        } else {
    ...
    onMouseDown(e) {
      ...
      const onBackground = !this.webGLStage._getObjectsUnderPoint(e.stageX, e.stageY, null, true);

      const event = { onBackground, x: e.stageX, y: e.stageY, originalEvent: e, worldPos: wop };
    ...
    onMouseUp(e) {
      if (this.disabled) { return; }
      createjs.lastMouseWorldPos = this.camera.screenToWorld({x: e.stageX, y: e.stageY});
      const onBackground = !this.webGLStage.hitTest(e.stageX, e.stageY);
      const event = { onBackground, x: e.stageX, y: e.stageY, originalEvent: e };
    ...
    onResize(e) {
      ...
      let scaleFactor = 1;
      if (this.options.stayVisible || features.codePlay) {
        let availableHeight = window.innerHeight;
        availableHeight -= ($('.ad-container').outerHeight() || 0);
        availableHeight -= ($('#game-area').outerHeight() || 0) - ($('#canvas-wrapper').outerHeight() || 0);
        if (features.codePlay) {
          const bannerHeight = ($('#codeplay-product-banner').height() || 0);
          availableHeight -= bannerHeight;
          if (availableHeight < newHeight) { scaleFactor = availableHeight / newHeight; }
        }
        if (availableHeight < newHeight) { scaleFactor = availableHeight / newHeight; }
      }
      ...
      if ((newWidth === oldWidth) && (newHeight === oldHeight) && !this.options.spectateGame) { return this.updateCodePlayMargin(); }
      if ((newWidth < 200) || (newHeight < 200)) { return this.updateCodePlayMargin(); }
      this.normalCanvas.add(this.webGLCanvas).attr({width: newWidth, height: newHeight});
      this.updateCodePlayMargin();
    ...
    updateCodePlayMargin() {
      if (!features.codePlay) { return; }
      const availableWidth = ((window.innerWidth * .57) - 200);
      const width = this.normalCanvas.attr('width');
      const margin = Math.max(availableWidth - width, 0);
      return this.normalCanvas.add(this.webGLCanvas).css('margin-left', margin/2);
    }
    ...
    updatePaths() {
      if (!this.options.paths || !this.heroLank) { return; }
      this.hidePaths();
      if (this.world.showPaths === 'never') { return; }
      const layerAdapter = this.lankBoss.layerAdapters['Path'];
      if (this.trailmaster == null) { this.trailmaster = new TrailMaster(this.camera, layerAdapter); }
    ...
    hidePathFinding() {
      if (this.navRectangles) { this.surfaceLayer.removeChild(this.navRectangles); }
      if (this.navPaths) { this.surfaceLayer.removeChild(this.navPaths); }
    ...
    showPathFinding() {
      ...
      if (!graph) { return this.surfaceLayer.updateLayerOrder(); }
    ...
    addMeshRectanglesToContainer(mesh, container) {
      return (() => {
        const result = [];
        for (let rect of Array.from(mesh)) {
          const shape = new createjs.Shape();
          const pos = this.camera.worldToSurface({x: rect.x, y: rect.y});
          const dim = this.camera.worldToSurface({x: rect.width, y: rect.height});
          shape.graphics
          .setStrokeStyle(3)
    ...
    destroy() {
      ...
      createjs.Sound.stop();
      for (let layer of Array.from(this.normalLayers)) { layer.destroy(); }
