...
module.exports = (SegmentedSprite = (function() {
  SegmentedSprite = class SegmentedSprite extends createjs.Container {
    ...
    goto(actionName, paused) {
      ...
      if (this.childMovieClips) {
        for (let mc of Array.from(this.childMovieClips)) { mc.inUse = false; }
      }
      ...
      if (action.animation) {
        ...
        for (let movieClip of Array.from(this.childMovieClips)) {
    ...
    buildMovieClip(animationName, mode, startPosition, loops) {
      ...
      for (let mc of Array.from(this.spriteSheet.mcPool[key])) {
        if (!mc.inUse) {
      ...
      const toSkip = {};
      for (let shape of Array.from(animData.shapes)) { toSkip[shape.bn] = true; }
      for (let graphic of Array.from(animData.graphics)) { toSkip[graphic.bn] = true; }
      ...
      for (let i = 0; i < animData.tweens.length; i++) {
        const tweenData = animData.tweens[i];
        let stopped = false;
        let tween = createjs.Tween;
        for (let func of Array.from(tweenData)) {
          const args = $.extend(true, [], (func.a));
  ...
  //          console.debug 'Did not dereference args:', args
    ...
    buildMovieClipShapes(localShapes) {
      const map = {};
      for (let localShape of Array.from(localShapes)) {
        var shape;
        if (localShape.im) {
    ...
    buildMovieClipContainers(localContainers) {
      const map = {};
      for (let localContainer of Array.from(localContainers)) {
        const outerContainer = new createjs.Container(this.spriteSheet);
        const innerContainer = new createjs.Sprite(this.spriteSheet);
        ...
        if ((innerContainer.currentFrame === 0) || this.usePlaceholders) {
          ...
          const bounds = this.thangType.get('raw').containers[localContainer.gn].b;
    ...
    buildMovieClipAnimations(localAnimations) {
      const map = {};
      for (let localAnimation of Array.from(localAnimations)) {
        const animation = this.buildMovieClip(localAnimation.gn, ...Array.from(localAnimation.a));
    ...
    dereferenceArgs(args, locals, toSkip) {
      for (let key in args) {
        ...
        if (locals[val]) {
          ...
          const res = this.dereferenceArgs(val, locals, toSkip);
    ...
    takeChildrenFromMovieClip(movieClip, recipientContainer) {
      return (() => {
        const result = [];
        for (let child of Array.from(movieClip.childrenCopy)) {
          if (child instanceof createjs.MovieClip) {
            const childRecipient = new createjs.Container(this.spriteSheet);
            this.takeChildrenFromMovieClip(child, childRecipient);
            for (let prop of ['regX', 'regY', 'rotation', 'scaleX', 'scaleY', 'skewX', 'skewY', 'x', 'y']) {
