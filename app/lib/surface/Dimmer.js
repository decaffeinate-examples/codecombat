...
module.exports = (Dimmer = (function() {
  Dimmer = class Dimmer extends CocoClass {
    ...
    dim() {
      ...
      for (let thangID in this.sprites) { const sprite = this.sprites[thangID]; sprite.setDimmed(true); }
    ...
    undim() {
      ...
      return (() => {
        const result = [];
        for (let thangID in this.sprites) {
          const sprite = this.sprites[thangID];
    ...
    updateDimMask() {
      ...
      for (let thangID in this.sprites) {
        const sprite = this.sprites[thangID];
        if ((!Array.from(this.highlightedThangIDs).includes(thangID)) && !(typeof sprite.isTalking === 'function' ? sprite.isTalking() : undefined)) { continue; }
        const sup = {x: sprite.sprite.x, y: sprite.sprite.y};
        const cap = this.camera.surfaceToCanvas(sup);
        const r = 50 * this.camera.zoom;  // TODO: find better way to get the radius based on the sprite's size
