...
class ViewLoadTimer {
  ...
  record() {
    ...
    .then(() => {
      ...
      console.log("Saw view load event", thatThereId, this.view.id);
      ...
      if (this.firstLoad) {
        ...
        if (VIEW_LOAD_LOG) {
          _.each({
            ...
            for (let entry of Array.from(entryList)) { console.log(`${entry.transferSize}: ${entry.name}`); }
      ...
      return window.timeSpendWaiting += totalTime;
    }).then(() => {
