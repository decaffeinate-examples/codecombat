...
module.exports.flattenDelta = (flattenDelta = function(delta, dataPath=null, deltaPath=null) {
  ...
  for (let deltaIndex in delta) {
    const childDelta = delta[deltaIndex];
    if (deltaIndex === '_t') { continue; }
    const dataIndex = affectingArray ? parseInt(deltaIndex.replace('_', '')) : deltaIndex;
...
module.exports.getConflicts = function(headDeltas, pendingDeltas) {
  ...
  for (let i = 0; i < paths.length; i++) {
    ...
    while ((i + offset) < paths.length) {
      ...
      for (let headMetaDelta of Array.from((headPathMap[path] || headPathMap[nextPath]))) {
        const headDelta = headMetaDelta.delta;
        for (let pendingMetaDelta of Array.from((pendingPathMap[path] || pendingPathMap[nextPath]))) {
          const pendingDelta = pendingMetaDelta.delta;
...
var groupDeltasByAffectingPaths = function(deltas) {
  ...
  for (let delta of Array.from(deltas)) {
    ...
    for (let path of Array.from(conflictPaths)) {
      metaDeltas.push({
        delta,
...
module.exports.pruneExpandedDeltasFromDelta = function(delta, expandedDeltas) {
  ...
  for (let expandedDelta of Array.from(expandedDeltas)) {
