...
const template = require('templates/test-view');
  ...
  TestView = class TestView extends RootView {
    ...
    initSpecFiles() {
      ...
      if (this.subPath) {
        ...
        return this.specFiles = ((() => {
          const result = [];
          for (let f of Array.from(this.specFiles)) {             if (_.string.startsWith(f, prefix)) {
              result.push(f);
            }
          }
          return result;
    ...
    static runTests(specFiles, demosOn, view) {
      ...
      if (demosOn) {
        ...
        jasmine.demoModal = _.once(modal => currentView.openModalView(modal));
      } else {
