...
const template = require('templates/play/modal/item-details-view');
...
module.exports = (ItemDetailsView = (function() {
  ItemDetailsView = class ItemDetailsView extends CocoView {
    ...
    onDocsLoaded(levelComponents) {
      for (let component of Array.from(levelComponents.models)) {
        for (let propDoc of Array.from(component.get('propertyDocumentation'))) {
    ...
    getRenderData() {
      ...
      if (this.item) {
        ...
        for (let prop of Array.from(stats.props)) {
          var left1;
          ...
          if (_.isString(description)) {
            ...
            description = description.replace(/#{([^.]+?)}/g, function(match, keyChain) {
              for (let componentConfig of Array.from(componentConfigs)) {
                var value;
