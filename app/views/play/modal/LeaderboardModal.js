...
const template = require('templates/play/modal/leaderboard-modal');
...
module.exports = (LeaderboardModal = (function() {
  LeaderboardModal = class LeaderboardModal extends ModalView {
    ...
    getRenderData(c) {
      ...
      c.submenus = [];
      for (let scoreType of Array.from((left = this.level.get('scoreTypes')) != null ? left : [])) {
        if (scoreType.type) { scoreType = scoreType.type; }
        for (let timespan of Array.from(this.timespans)) {
    ...
    afterRender() {
      ...
      for (let scoreTypeIndex = 0; scoreTypeIndex < iterable.length; scoreTypeIndex++) {
        let scoreType = iterable[scoreTypeIndex];
        if (scoreType.type) { scoreType = scoreType.type; }
        for (let timespanIndex = 0; timespanIndex < this.timespans.length; timespanIndex++) {
          const timespan = this.timespans[timespanIndex];
          const submenuView = new LeaderboardTabView({scoreType, timespan, level: this.level});
    ...
    onTabShown(e) {
      ...
      return (() => {
        ...
        for (let subviewKey in this.subviews) {
          const otherSubview = this.subviews[subviewKey];
