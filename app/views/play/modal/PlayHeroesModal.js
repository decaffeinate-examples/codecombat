...
const template = require('templates/play/modal/play-heroes-modal');
...
module.exports = (PlayHeroesModal = (function() {
  PlayHeroesModal = class PlayHeroesModal extends ModalView {
    ...
    onHeroesLoaded() {
      ...
      for (let hero of Array.from(this.heroes.models)) { this.formatHero(hero); }
    ...
    formatHero(hero) {
      ...
      hero.purchasable = hero.locked && (me.isPremium() || me.allowStudentHeroPurchase());
    ...
    initCodeLanguageList(hadEverChosenHero) {
      if (application.isIPadApp) {
        ...
        if (me.isAdmin() || !application.isProduction()) {
          this.codeLanguageList.push({id: 'java', name: `Java (${$.i18n.t('choose_hero.experimental')})`});
          this.codeLanguageList.push({id: 'cpp', name: `C++ (${$.i18n.t('choose_hero.experimental')})`});
          return this.codeLanguageList.push({id: 'lua', name: `Lua (${$.i18n.t('choose_hero.experimental')})`});
        }
      }
    }
    ...
    destroy() {
      ...
      for (let heroIndex in this.stages) {
        const stage = this.stages[heroIndex];
        createjs.Ticker.removeEventListener("tick", stage);
        stage.removeAllChildren();
      }
      for (let layer of Array.from(this.layers)) { layer.destroy(); }
