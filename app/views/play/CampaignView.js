...
require('three');
const ParticleMan = require('core/ParticleMan');
...
const codePlay = require('lib/code-play');
const MineModal = require('views/core/MineModal'); // Minecraft modal
const CodePlayCreateAccountModal = require('views/play/modal/CodePlayCreateAccountModal');
...
module.exports = (CampaignView = (function() {
  CampaignView = class CampaignView extends RootView {
    ...
    constructor(options, terrain) {
      ...
      if (_.string.startsWith(this.terrain, "ozaria") && (!me.showOzariaCampaign() || !this.editorMode)) {
      ...
      this.listenToOnce(this.earnedAchievements, 'sync', function() {
        ...
        return (() => {
          const result = [];
          for (let m of Array.from(this.earnedAchievements.models)) {
            var loadedEarned;
            ...
            result.push((() => {
              ...
              for (var group of ['heroes', 'levels', 'items']) {
                ...
                result1.push((() => {
                  const result2 = [];
                  for (let reward of Array.from(loadedEarned[group])) {
          ...
          if (this.courseInstance.get('classroomID')) {
            ...
            return this.listenToOnce(this.classroom, 'sync', () => {
              ...
              return this.listenToOnce(this.courseLevels, 'sync', () => {
                ...
                const classroomCourse = _.find(currentView.classroom.get('courses'), {_id:currentView.course.id});
                const levelPositions = {};
                for (let level of Array.from(classroomCourse.levels)) {
    ...
    onLoaded() {
      ...
      if (!me.get('hourOfCode') && this.terrain) {
        ...
        if (features.codePlay) {
          if (me.get('anonymous') && (me.get('lastLevel') === 'true-names') && (me.level() < 5)) {
            return this.openModalView(new CodePlayCreateAccountModal());
          }
        } else if (me.get('name') && (needle = me.get('lastLevel'), ['forgetful-gemsmith', 'signs-and-portents', 'true-names'].includes(needle)) &&
    ...
    buildLevelScoreMap() {
      for (let session of Array.from(this.sessions.models)) {
        ...
        if (!levels) { return; }
        const levelOriginal = __guard__(session.get('level'), x => x.original);
    ...
    getRenderData(context) {
      ...
      if (me.freeOnly()) {
        context.levels = _.reject(context.levels, function(level) {
          if (features.codePlay && codePlay.canPlay(level.slug)) { return false; }
        ...
        for (campaign of Array.from(this.campaigns.models)) {
          if (campaign.get('slug') !== 'auditions') {
            ...
            if (this.sessions != null ? this.sessions.loaded : undefined) {
              ...
              if (me.freeOnly()) {
                levels = _.reject(levels, function(level) {
                  if (features.codePlay && codePlay.canPlay(level.slug)) { return false; }
                  return level.requiresSubscription;
                });
              }
    ...
    generateCompletionRates() {
      ...
      return (() => {
        const result = [];
        for (let level of Array.from((this.campaign != null ? this.campaign.renderedLevels : undefined) != null ? (this.campaign != null ? this.campaign.renderedLevels : undefined) : [])) {
    ...
    onLevelCompletionsLoaded(level, data) {
      ...
      let started = 0;
      let finished = 0;
      for (let day of Array.from(data)) {
      ...
      return (() => {
        ...
        for (let levelIndex = 0; levelIndex < orderedLevels.length; levelIndex++) {
          ...
          if (!this.isClassroom() && ((this.campaign != null ? this.campaign.get('type') : undefined) !== 'hoc')) {
            if (level.requiresSubscription && !features.codePlay) { level.color = 'rgb(80, 130, 200)'; }
...
`;
    ...
    determineNextLevel(orderedLevels) {
      ...
      const findNextLevel = (level, practiceOnly) => {
        for (let nextLevelOriginal of Array.from(level.nextLevels)) {
    ...
    createLines() {
      return Array.from((this.campaign != null ? this.campaign.renderedLevels : undefined) != null ? (this.campaign != null ? this.campaign.renderedLevels : undefined) : []).map((level) =>
        (() => {
          const result = [];
          for (let nextLevelOriginal of Array.from(level.nextLevels != null ? level.nextLevels : [])) {
            var nextLevel;
    ...
    applyCampaignStyles() {
      ...
      if (backgroundColor = this.campaign.get('backgroundColor')) {
        ...
        for (let pos of ['top', 'right', 'bottom', 'left']) {
    ...
    testParticles() {
      if (!(this.campaign != null ? this.campaign.loaded : undefined) || !$.browser.chrome) { return; }  // Sometimes this breaks in non-Chrome browsers, according to A/B tests.
      if (this.campaign.get('type') === 'hoc') { return; }
      if (this.particleMan == null) { this.particleMan = new ParticleMan(); }
      this.particleMan.removeEmitters();
      this.particleMan.attach(this.$el.find('.map'));
      return (() => {
        const result = [];
        for (let level of Array.from(this.campaign.renderedLevels != null ? this.campaign.renderedLevels : {})) {
          if (level.hidden && (this.campaign.levelIsPractice(level) || this.campaign.levelIsAssessment(level) || !level.unlockedInSameCampaign)) { continue; }
          const terrain = this.terrain.replace('-branching-test', '').replace(/(campaign-)?(game|web)-dev-\d/, 'forest').replace(/(intro|game-dev-hoc)/, 'dungeon');
          const particleKey = ['level', terrain];
          if (level.type && !(['hero', 'course'].includes(level.type))) { particleKey.push(level.type); }  // Would use isType, but it's not a Level model
          if (level.replayable) { particleKey.push('replayable'); }
          if (level.requiresSubscription) { particleKey.push('premium'); }
          if (['kithgard-gates', 'siege-of-stonehold', 'clash-of-clones', 'summits-gate'].includes(level.slug)) { particleKey.push('gate'); }
          if (level.unlocksHero && !level.unlockedHero) { particleKey.push('hero'); }
          //particleKey.push 'item' if level.slug is 'robot-ragnarok'  # TODO: generalize
          if (particleKey.length === 2) { continue; }  // Don't show basic levels
          if (!level.hidden && !_.intersection(particleKey, ['item', 'hero-ladder', 'replayable']).length) { continue; }
          result.push(this.particleMan.addEmitter(level.position.x / 100, level.position.y / 100, particleKey.join('-')));
        }
        return result;
      })();
    }
    ...
    onClickStartLevel(e) {
      ...
      const freeAccessLevels =  ['dungeons-of-kithgard', 'gems-in-the-deep', 'shadow-guard', 'enemy-mine', 'cell-commentary', 'kithgard-librarian', 'the-prisoner', 'fire-dancing', 'haunted-kithmaze', 'signs-and-portents', 'true-names'];
      const requiresSubscription = level.requiresSubscription || (me.isOnPremiumServer() && !(Array.from(freeAccessLevels).includes(level.slug)));
      const canPlayAnyway = _.any([
        !this.requiresSubscription,
        level.adventurer,
        this.levelStatusMap[level.slug],
        (features.codePlay && codePlay.canPlay(level.slug)),
    ...
    onWindowResize(e) {
      let adContainerHeight, iPadHeight, resultingHeight, resultingWidth;
      ...
      let pageHeight = this.$el.height();
      if (adContainerHeight = $('.ad-container').outerHeight()) { pageHeight -= adContainerHeight; }
      ...
      if (this.particleMan) { return this.testParticles(); }
    }
    ...
    preloadTopHeroes() {
      ...
      return (() => {
        const result = [];
        for (let heroID of ['captain', 'knight']) {
    ...
    checkForUnearnedAchievements() {
      if (!this.campaign || !currentView.sessions) { return; }
      ...
      .done(achievements => {
        if (this.destroyed) { return; }
        const sessionsComplete = _(currentView.sessions.models)
        ...
        return (() => {
          ...
          for (var achievement of Array.from(levelAchievements)) {
            ...
            result.push((() => {
              const result1 = [];
              for (let levelOriginal of Array.from(achievement.rewards.levels)) {
    ...
    applyCourseLogicToLevels(orderedLevels) {
      ...
      for (let levelIndex = 0; levelIndex < courseOrder.length; levelIndex++) {
        const level = courseOrder[levelIndex];
        ...
        if (lockedByTeacher && me.showCourseProgressControl()) {
          level.locked = true;
        }
    ...
    shouldShow(what) {
      ...
      if (what === 'classroom-level-play-button') {
        const isValidStudent = (me.isStudent() && __guard__(me.get('courseInstances'), x => x.length));
        const isValidTeacher = me.isTeacher();
        return (isValidStudent || isValidTeacher) && !application.getHocCampaign();
      }

      if (features.codePlay && ['clans', 'settings'].includes(what)) {
        return false;
      }
      ...
      if (what === 'codeplay-ads') {
        return !me.finishedAnyLevels() && serverConfig.showCodePlayAds && !features.noAds && (me.get('role') !== 'student');
      }
      ...
      if (['status-line'].includes(what)) {
        return me.showGemsAndXp() || !isStudentOrTeacher;
      }
      ...
      if (['back-to-classroom'].includes(what)) {
        return isStudentOrTeacher && !application.getHocCampaign();
      }
      ...
      if (['buy-gems'].includes(what)) {
        return !(isIOS || me.freeOnly() || isStudentOrTeacher || !me.canBuyGems() || (application.getHocCampaign() && me.isAnonymous()));
      }

      if (['premium'].includes(what)) {
        return !(me.isPremium() || isIOS || me.freeOnly() || isStudentOrTeacher || (application.getHocCampaign() && me.isAnonymous()));
      }
