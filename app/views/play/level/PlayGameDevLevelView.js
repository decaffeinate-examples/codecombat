...
module.exports = (PlayGameDevLevelView = (function() {
  PlayGameDevLevelView = class PlayGameDevLevelView extends RootView {
    static initClass() {
      ...
      this.prototype.template = require('templates/play/level/play-game-dev-level-view');
    ...
    initialize(options, sessionID) {
      ...
      return new Promise((accept,reject) => this.session.fetch({ cache: false }).then(accept, reject)).then(sessionData => {
        ...
        if (window.tracker != null) {
          window.tracker.trackEvent('Play GameDev Level - Load', this.eventProperties, ['Mixpanel']);
        }
        ...
        return this.willUpdateFrontEnd = true;
      }).catch(e => {
        if (e.stack) { throw e; }
    ...
    onClickPlayButton() {
      ...
      if (window.tracker != null) {
        window.tracker.trackEvent(action, this.eventProperties, ['Mixpanel']);
      }
    ...
    onClickCopyURLButton() {
      ...
      this.tryCopy();
      return (window.tracker != null ? window.tracker.trackEvent('Play GameDev Level - Copy URL', this.eventProperties, ['Mixpanel']) : undefined);
    }

    onClickPlayMoreCodeCombatButton() {
      return (window.tracker != null ? window.tracker.trackEvent('Play GameDev Level - Click Play More CodeCombat', this.eventProperties, ['Mixpanel']) : undefined);
    }
    ...
    getLevelName() {
      return this.levelName != null ? this.levelName : this.level.get('name');
    }
