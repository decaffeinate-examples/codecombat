...
module.exports = (CourseVictoryModal = (function() {
  CourseVictoryModal = class CourseVictoryModal extends ModalView {
    ...
    initialize(options) {
      ...
      this.capstoneStage = options.capstoneStage;
      ...
      if (!utils.ozariaCourseIDs.includes(this.courseID)) {
    ...
    onLoaded() {
      ...
      if (me.showGemsAndXp() && (this.achievements.length > 0)) {
        ...
        for (let achievement of Array.from(this.achievements.models)) {
      ...
      } else if (utils.ozariaCourseIDs.includes(this.courseID)) {  // if it is ozaria course and there is no course instance, load campaign so that we can calculate next levels
    ...
    getNextLevelOzaria() {
      ...
      if (this.classroom && this.levelSessions) { // fetch next level based on sessions and classroom levels
        ...
        if (currentLevel.isPlayedInStages && this.capstoneStage) { // @capstoneStage comes from PlayLevelView's query params
          currentLevelStage = this.capstoneStage;
        }
        const nextLevelData = getNextLevelForLevel(currentLevel, currentLevelStage) || {};
