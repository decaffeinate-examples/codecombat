...
const template = require('templates/play/play-level-view');
...
module.exports = (PlayLevelView = (function() {
  PlayLevelView = class PlayLevelView extends RootView {
    ...
    onWorldNecessitiesLoaded() {
      ...
      const randomTeam = this.world != null ? this.world.teamForPlayer() : undefined;  // If no team is set, then we will want to equally distribute players to teams
    ...
    grabLevelLoaderData() {
      ...
      if (this.level.isType('hero', 'hero-ladder', 'hero-coop', 'course', 'course-ladder', 'game-dev')) { this.$el.addClass('hero'); }  // TODO: figure out what this does and comment it
    ...
    loadOpponentTeam(myTeam) {
      ...
      if ((!this.session.get('teamSpells')) && (this.otherSession != null ? this.otherSession.get('teamSpells') : undefined)) {
    ...
    initGoalManager() {
      ...
      if ((((this.session != null ? this.session.get('codeLanguage') : undefined) === 'cpp') || (__guard__(me.get('aceConfig'), x => x.language) === 'cpp')) && Array.isArray(this.level.get('goals'))) {
        this.level.get('goals').forEach(goal => {
          if (__guard__(goal != null ? goal.linesOfCode : undefined, x1 => x1.humans) && (typeof goal.linesOfCode.humans === 'number')) {
            return goal.linesOfCode.humans += 2;
          }
        });
      }
    ...
    insertSubviews() {
      ...
      this.insertSubView(this.controlBar = new ControlBarView({worldName: utils.i18n(this.level.attributes, 'name'), session: this.session, level: this.level, supermodel: this.supermodel, courseID: this.courseID, courseInstanceID: this.courseInstanceID}));
    ...
    onSessionLoaded(e) {
      ...
      if (e.level.isType('hero', 'hero-ladder', 'hero-coop') && !_.size((left1 = __guard__(e.session.get('heroConfig'), x => x.inventory)) != null ? left1 : {}) && (e.level.get('assessment') !== 'open-ended')) {
        ...
        return _.defer(() => {
          let left2;
          if (_.size((left2 = __guard__(e.session.get('heroConfig'), x1 => x1.inventory)) != null ? left2 : {})) { return; }
    ...
    simulateNextGame() {
      ...
      if (this.level.isType('course-ladder', 'hero-ladder')) { simulatorOptions.levelID = this.level.get('slug'); }
    ...
    onLevelReloadFromData(e) {
      ...
      if (isReload) {
        ...
        for (let url in this.supermodel.models) { const model = this.supermodel.models[url]; if (!e.supermodel.models[url]) { e.supermodel.registerModel(model); } }
      }
    ...
    onShowVictory(e) {
      if (e == null) { e = {}; }
      if (!this.level.isType('hero', 'hero-ladder', 'hero-coop', 'course', 'course-ladder', 'game-dev', 'web-dev')) { $('#level-done-button').show(); }
      ...
      if (!this.observing && (victoryTime > (10 * 1000))) {   // Don't track it if we're reloading an already-beaten level
        if (application.tracker != null) {
    ...
    showVictory(options) {
      ...
      let ModalClass = this.level.isType('hero', 'hero-ladder', 'hero-coop', 'course', 'course-ladder', 'game-dev', 'web-dev') ? HeroVictoryModal : VictoryModal;
      ...
      if ((this.level.get("slug") === "code-play-share") && this.level.get('shareable')) {
        const hocModal = new HoC2018VictoryModal({
          shareURL: `${window.location.origin}/play/${this.level.get('type')}-level/${this.session.id}`,
    ...
    onRestartLevel() {
      ...
      if (!this.observing) { return (application.tracker != null ? application.tracker.trackEvent('Confirmed Restart', {category: 'Play Level', level: this.level.get('name'), label: this.level.get('name')}) : undefined); }
    }

    onInfiniteLoop(e) {
      if (!e.firstWorld || (e.god !== this.god)) { return; }
      this.openModalView(new InfiniteLoopModal({nonUserCodeProblem: e.nonUserCodeProblem}));
      if (!this.observing) { return (application.tracker != null ? application.tracker.trackEvent('Saw Initial Infinite Loop', {category: 'Play Level', level: this.level.get('name'), label: this.level.get('name')}) : undefined); }
    }
    ...
    onNewWorld(e) {
      ...
      if (this.level.isType('game-dev')) { return this.session.updateKeyValueDb(e.keyValueDb); }
    }
    ...
    onRealTimePlaybackEnded(e) {
      ...
      this.onWindowResize();
      if (this.level.isType('game-dev')) { this.session.saveKeyValueDb(); }
