  ...
  Spell = class Spell {
    ...
    constructor(options) {
      ...
      if (this.canRead()) {  // We can avoid creating these views if we'll never use them.
        this.view = new SpellView({spell: this, level: options.level, session: this.session, otherSession: this.otherSession, worker: this.worker, god: options.god, supermodel: this.supermodel, levelID: options.levelID});
    ...
    setLanguage(language) {
      ...
      if ((this.language === 'java') && !this.languages[this.language]) {
        const lines = (this.languages.javascript != null ? this.languages.javascript : '').split('\n');
...
`;
      ...
      if ((this.language === 'cpp') && !this.languages[this.language]) {
        this.languages.cpp = utils.translatejs2cpp(this.languages.javascript);
      }
      ...
      if (/loop/.test(this.originalSource) && this.level.isType('course', 'course-ladder')) {
        ...
        return this.originalSource = (() => { switch (this.language) {
          ...
          case 'javascript': case 'java': case 'cpp': return this.originalSource.replace(/loop {/, 'while (true) {');
...
`.replace(/<p>(.*?)<\/p>/gi, '$1');
    ...
    createProblemContext(thang) {
      ...
      for (let key in (thang.world != null ? thang.world.thangMap : undefined)) {
        const value = (thang.world != null ? thang.world.thangMap : undefined)[key];
      ...
      if (thang.programmableProperties != null) {
        for (let prop of Array.from(thang.programmableProperties)) {
