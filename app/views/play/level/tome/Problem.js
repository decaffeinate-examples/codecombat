  ...
  Problem = class Problem {
    ...
    constructor({ aether, aetherProblem, ace1, isCast, levelID, error, userCodeHasChangedSinceLastCast }) {
      ...
      if (this.aetherProblem) {
        ...
        ({ level: this.level, range: this.range, message: this.message, hint: this.hint, userInfo: this.userInfo } = this.aetherProblem);
    ...
    translate(msg) {
      ...
      if ((needle = i18n.lng(), ['en', 'en-US'].includes(needle))) { return msg; }
      ...
      for (let keySet of [prefixKeys, Object.keys(_.omit(en.esper), prefixKeys)]) {
        for (let translationKey of Array.from(keySet)) {
          let didTranslate;
          const englishString = en.esper[translationKey];
          const regex = this.makeTranslationRegex(englishString);
