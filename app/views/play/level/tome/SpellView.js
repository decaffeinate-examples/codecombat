...
const template = require('templates/play/level/tome/spell');
...
module.exports = (SpellView = (function() {
  SpellView = class SpellView extends CocoView {
    ...
    createACE() {
      ...
      this.initAutocomplete(aceConfig.liveCompletion != null ? aceConfig.liveCompletion : true);
        ...
        update: (html, markerLayer, session, config) => {
          ...
          return (() => {
            ...
            for (let row = 0, end = this.aceSession.getLength(), asc = 0 <= end; asc ? row <= end : row >= end; asc ? row++ : row--) {
              ...
              const xstart = startOfRow(row);
              if (language === 'python') {
                var asc1, crow, end1;
                const requiredIndent = new RegExp('^' + new Array(Math.floor((xstart / 4) + 1)).join('(    |\t)') + '(    |\t)+(\\S|\\s*$)');
              ...
              const rstart = this.aceSession.documentToScreenPosition(docRange.start.row, docRange.start.column);
              const rend = this.aceSession.documentToScreenPosition(docRange.end.row, docRange.end.column);
              const range = new Range(rstart.row, rstart.column, rend.row, rend.column);
              const level = Math.floor(xstart / 4);
              const color = colors[level % colors.length];
              const bw = 3;
              const to = markerLayer.$getTop(range.start.row, config);
              const t = markerLayer.$getTop(range.start.row + 1, config);
              const h = config.lineHeight * (range.end.row - range.start.row);
              const l = markerLayer.$padding + (xstart * config.characterWidth);
              // w = (data.i - data.b) * config.characterWidth
              const w = 4 * config.characterWidth;
              const fw = config.characterWidth * ( this.aceSession.getScreenLastRowColumn(range.start.row) - xstart );

              result.push(html.push(`\
<div style=
...
`
    ...
    lockDefaultCode(force) {
      ...
      const intersects = () => {
        for (let range of Array.from(this.readOnlyRanges)) { if (this.ace.getSelectionRange().intersects(range)) { return true; } }
        return false;
      };
      ...
      const intersectsLeft = () => {
        ...
        for (let range of Array.from(this.readOnlyRanges)) { if (leftRange.intersects(range)) { return true; } }
        return false;
      };
      ...
      const intersectsRight = () => {
        ...
        for (let range of Array.from(this.readOnlyRanges)) { if (rightRange.intersects(range)) { return true; } }
        return false;
      };
      ...
      if (this.lockedCodeMarkerIDs != null) {
        for (let marker of Array.from(this.lockedCodeMarkerIDs)) { this.aceSession.removeMarker(marker); }
      }
      ...
      for (let range of Array.from(this.readOnlyRanges)) {
    ...
    updateLines() {
      ...
      if (screenLineCount !== this.lastScreenLineCount) {
        ...
        const tomeHeight = $('#tome-view').innerHeight();
        const spellTopBarHeight = $('#spell-top-bar-view').outerHeight();
        const spellToolbarHeight = $('.spell-toolbar-view').outerHeight();
        if (this.spellPaletteHeight == null) { this.spellPaletteHeight = 75; }
        const spellPaletteAllowedHeight = Math.min(this.spellPaletteHeight, tomeHeight / 3);
        const maxHeight = tomeHeight - spellTopBarHeight - spellToolbarHeight - spellPaletteAllowedHeight;
        const minHeight = Math.max(8, (Math.min($("#canvas-wrapper").outerHeight(),$("#level-view").innerHeight() - 175) / lineHeight) - 2);
        const linesAtMaxHeight = Math.floor(maxHeight / lineHeight);
        const lines = Math.max(minHeight, Math.min(screenLineCount + 2, linesAtMaxHeight));
    ...
    fetchToken(source, language) {
      ...
      const service = __guard__(typeof window !== 'undefined' && window !== null ? window.localStorage : undefined, x => x.kodeKeeperService) || "/service/parse-code";
    ...
    preload() {
      ...
      return (() => {
        const result = [];
        for (let key in oldSpellThangAether) {
          const value = oldSpellThangAether[key];
    ...
    highlightCurrentLine(flow) {
      ...
      for (callNumber = 0; callNumber < states.length; callNumber++) {
        const callState = states[callNumber];
        ...
        executed.push([]);
        for (let statementNumber = 0; statementNumber < callState.statements.length; statementNumber++) {
      ...
      for (let i = 0; i < iterable.length; i++) {
        var markerType;
        state = iterable[i];
        const [start, end] = Array.from(state.range);
        const clazz = i === (lastExecuted.length - 1) ? 'executing' : 'executed';
        if (clazz === 'executed') {
        ...
        if (executedRows[start.row] && (this.decoratedGutter[start.row] !== clazz)) {
          ...
          const $cinematicParent = $('#cinematic-code-display');
          let highlightedIndex = 0;
          for (let start1 = end.row - 3, sourceLineNumber = start1, end2 = end.row + 3, asc1 = start1 <= end2; asc1 ? sourceLineNumber <= end2 : sourceLineNumber >= end2; asc1 ? sourceLineNumber++ : sourceLineNumber--) {
            const codeLine = _.string.rtrim(this.aceDoc.$lines[sourceLineNumber]);
            const $codeLineEl = $cinematicParent.find(`.code-line-${highlightedIndex++}`);
    ...
    highlightEntryPoints() {
      ...
      return (() => {
        ...
        for (let index = 0; index < lines.length; index++) {
          ...
          for (let i of [0, 4, 8, 12, 16]) { session.removeGutterDecoration(index, `entry-point-indent-${i}`); }

          const lineHasComment = this.singleLineCommentRegex().test(line);
          const lineHasCode = line.trim()[0] && !this.singleLineCommentOnlyRegex().test(line);
          const lineIsBlank = /^[ \t]*$/.test(line);
          const lineHasExplicitMarker = line.indexOf('∆') !== -1;

          const originalLine = originalLines[index];
          let lineHasChanged = line !== originalLine;

          let isEntryPoint = lineIsBlank && previousLineHadComment && !previousLineHadCode && pastIntroComments;
          if (isEntryPoint && lineHasChanged) {
            ...
            const movedIndex = originalLines.indexOf(previousLine);
    ...
    checkRequiredCode() {
      ...
      return (() => {
        const result = [];
        for (let requiredCodeFragment of Array.from(requiredCodeFragments)) {
    ...
    checkSuspectCode() {
      ...
      for (let suspectCodeFragment of Array.from(suspectCodeFragments)) {
        const pattern = new RegExp(suspectCodeFragment.pattern, 'm');
      ...
      for (let lastDetectedSuspectCodeFragmentName of Array.from(this.lastDetectedSuspectCodeFragmentNames != null ? this.lastDetectedSuspectCodeFragmentNames : [])) {
    ...
    destroy() {
      $(this.ace != null ? this.ace.container : undefined).find('.ace_gutter').off('click', '.ace_error, .ace_warning, .ace_info', this.onAnnotationClick);
      $(this.ace != null ? this.ace.container : undefined).find('.ace_gutter').off('click', this.onGutterClick);
      if (this.firepad != null) {
        this.firepad.dispose();
      }
      for (let command of Array.from(this.aceCommands)) { if (this.ace != null) {
