...
module.exports = function(SnippetManager, autoLineEndings) {
  ...
  SnippetManager.insertSnippet = function(editor, snippet) {
    ...
    if (cursor.column > 0) {
      ...
      if (prevWord.length > 0) {
        ...
        if (prevWordIndex === 0) {
          ...
          for (let completer of Array.from(editor.completers)) {
            if (completer.completions != null) {
              for (let completion of Array.from(completer.completions)) {
    ...
    if (match) { afterIndex += match[0].length; }
    const afterRange = new Range(cursor.row, cursor.column, cursor.row, afterIndex);
  ...
  return {
    getCompletions(editor, session, pos, prefix, callback) {
      ...
      if ((!(fullPrefixParts.length < 3) || !/^(hero|self|this|@|db|game|ui)$/.test(fullPrefixParts[0]) ) && !/^\s*$/.test(beginningOfLine)) {
      ...
      SnippetManager.getActiveScopes(editor).forEach(function(scope) {
        ...
        return (() => {
          const result = [];
          for (let s of Array.from(snippets)) {
            var left;
...
var scrubSnippet = function(snippet, caption, line, input, pos, lang, autoLineEndings, captureReturn) {
  ...
  if (prefixStart = snippet.toLowerCase().indexOf(input.toLowerCase()) > -1) {
    ...
    if (snippet.indexOf(lineSuffix) < 0) { lineSuffix = ''; }
