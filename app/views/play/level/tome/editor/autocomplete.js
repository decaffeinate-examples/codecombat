  ...
  Autocomplete = class Autocomplete {
    ...
    activateCompleter(comp) {
      if (Array.isArray(comp)) {
        ...
        return (() => {
          const result = [];
          for (let type in this.completers) {
            const comparator = this.completers[type];
      ...
      return ace.config.loadModule('ace/ext/language_tools', () => {
        ...
        return ace.config.loadModule(snippetModulePath, m => {
          if (m != null) {
            ...
            for (let s of Array.from(snippets)) { m.snippets.push(s); }
    ...
    doLiveCompletion(e) {
      ...
      if ((e.command.name === "backspace") || (e.command.name === "insertstring")) {
        ...
        if ((token != null) && !['comment', 'string'].includes(token.type)) {
          const prefix = this.getCompletionPrefix(editor);
          ...
          if (prefix) {
            ...
            if (__guard__(__guard__(editor.completer != null ? editor.completer.completions : undefined, x2 => x2.filtered), x1 => x1.length) > 20) {
              editor.completer.detach();
    ...
    getCompletionPrefix(editor) {
      ...
      if (prefix == null) { prefix = util.retrievePrecedingIdentifier(line, pos.column); }
      return prefix;
    }

    addCodeCombatSnippets(level, spellView, e) {
      let attackEntry, content, entry, left, name;
      ...
      for (let group in e.propGroups) {
        const props = e.propGroups[group];
        for (let prop of Array.from(props)) {
          ...
          const doc = _.find(((left1 = e.allDocs['__' + prop]) != null ? left1 : []), function(doc) {
          ...
          if (__guard__(doc != null ? doc.snippets : undefined, x2 => x2[e.language])) {
            ({
              name
            } = doc);
            const replacement = _.find(autocompleteReplacement, el => el.name === name);
            content = __guard__(__guard__(replacement != null ? replacement.snippets : undefined, x4 => x4[e.language]), x3 => x3.code) || doc.snippets[e.language].code;
            ...
            if (doc.userShouldCaptureReturn) {
              ...
              entry.captureReturn = (() => { switch (e.language) {
                case 'javascript': case 'java': return 'var ' + varName + ' = ';
                case 'cpp': return 'auto ' + varName + ' = ';  // TODO: be smarter about return types when we can?
