 ...
 */
let DocFormatter;
const popoverTemplate = require('templates/play/level/tome/spell_palette_entry_popover');
...
module.exports = (DocFormatter = class DocFormatter {
  ...
  fillOutDoc() {
    ...
    for (let {obj, prop} of Array.from(toTranslate)) {
      // Translate into chosen code language.
      var originalVal, val;
      if (['java', 'cpp'].includes(this.options.language) && !(obj[prop] != null ? obj[prop][this.options.language] : undefined) && (obj[prop] != null ? obj[prop].javascript : undefined)) {
  ...
  replaceSimpleLoops() {
    ...
    return (() => {
      const result = [];
      for (let field of ['example', 'description']) {
        const [simpleLoop, whileLoop] = Array.from((() => { switch (this.options.language) {
  ...
  inferCooldowns() {
    ...
    for (let prop of ['range', 'radius', 'duration', 'damage']) {
      const v = owner[_.string.camelize(actionName + _.string.capitalize(prop))];
