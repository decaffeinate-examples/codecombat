...
const template = require('templates/play/level/game_dev_track_view');
  ...
  GameDevTrackView = class GameDevTrackView extends CocoView {
    ...
    onFrameChanged(e) {
      ...
      if (e.world.synchronous) {
        let hero;
        for (let thang of Array.from(e.world.thangs)) {
        ...
        for (let key in thangStateMap) {
          var propIndex;
          const thangState = thangStateMap[key];
          if (!thangState.trackedPropertyKeys) { continue; }
          const trackedPropNamesIndex = thangState.trackedPropertyKeys.indexOf('uiTrackedProperties');
          if (trackedPropNamesIndex !== -1) {
            const trackedPropNames = thangState.props[trackedPropNamesIndex];
            if (trackedPropNames) {
          ...
          const trackedObjNamesIndex = thangState.trackedPropertyKeys.indexOf('objTrackedProperties');
          if (trackedObjNamesIndex === -1) { continue; }
          const trackedObjNames = thangState.props[trackedObjNamesIndex];
