...
const template = require('templates/play/menu/inventory-modal');
const buyGemsPromptTemplate = require('templates/play/modal/buy-gems-prompt');
const earnGemsPromptTemplate = require('templates/play/modal/earn-gems-prompt');
const subscribeForGemsPrompt = require('templates/play/modal/subscribe-for-gems-prompt');
  ...
  InventoryModal = class InventoryModal extends ModalView {
    ...
    initialize(options) {
      if (application.getHocCampaign() === 'game-dev-hoc-2') {
        ...
        for (let item of Array.from(baseItems)) {
          var needle;
    ...
    onItemsLoaded() {
      ...
      for (let itemGroup of Array.from(_.values(this.itemGroups))) { itemGroup.comparator = (function(m) { let left;
    ...
    sortItem(item, equipped) {
      ...
      for (let heroClass of Array.from(item.getAllowedHeroClasses())) {
      ...
      if ((gearSlugs[item.get('original')] !== 'tarnished-bronze-breastplate') || !inCampaignView || (this.options.level.get('slug') !== 'the-raised-sword')) {
        ...
        for (let slot of Array.from(item.getAllowedSlots())) {
          var requiredItems;
    ...
    getRenderData(context) {
      ...
      for (let slot in context.equipment) { const itemOriginal = context.equipment[slot]; context.equipment[slot] = this.items.findWhere({original: itemOriginal}); }
    ...
    setUpDraggableEventsForEquippedArea() {
      ...
      for (let itemSlot of Array.from(this.$el.find('.item-slot'))) {
    ...
    equipSelectedItem() {
      ...
      for (let allowedSlot of Array.from(allowedSlots)) {
    ...
    getCurrentEquipmentConfig() {
      const config = {};
      for (let slot of Array.from(this.$el.find('.item-slot'))) {
    ...
    requireLevelEquipment() {
      ...
      console.log("@remainingRequiredEquipment.length: " + this.remainingRequiredEquipment.length);
    ...
    calculateRequiredGearPerSlot() {
      ...
      for (let item of Array.from(this.items.models)) {
        ...
        for (let slot of Array.from(item.getAllowedSlots())) {
          ...
          for (let prop of Array.from(requiredPropertiesOnThisItem)) { if (!Array.from(requiredPropertiesPerSlot[slot]).includes(prop)) { requiredPropertiesPerSlot[slot].push(prop); } }
        }
      }
    ...
    unequipLevelRestrictedItems(equipment) {
      ...
      const restrictedGear = this.calculateRestrictedGearPerSlot();
      for (let slot in restrictedGear) {
    ...
    onUnlockButtonClicked(e) {
      ...
      if (!affordable) {
        ...
        for (let otherItem of Array.from(this.items.models)) { this.sortItem(otherItem, equipped); }
    ...
    addDollImage(slot, dollImages, heroClass, gender, item) {
      ...
      for (let imageKey of Array.from(imageKeys)) {
        const imageURL = dollImages[imageKey];
        if (!imageURL) {
          console.log(`Hmm, should have ${slot} ${imageKey} paper doll image, but don't have it.`);
        } else {
          const imageEl = $('<img>').attr('src', `/file/${imageURL}`).addClass(`doll-image ${slot} ${heroClass} ${gender} ${_.string.underscored(imageKey).replace(/_/g, '-')}`).attr('draggable', false);
