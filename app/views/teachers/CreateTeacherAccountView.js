...
const loadSegment = require('core/services/segment');
...
module.exports = (CreateTeacherAccountView = (function() {
  CreateTeacherAccountView = class CreateTeacherAccountView extends RootView {
    static initClass() {
      ...
      this.prototype.template = require('templates/teachers/create-teacher-account-view');
    ...
    initialize() {
      ...
      if (!this.segmentLoaded) { return loadSegment(); }
    }
    ...
    afterRender() {
      ...
      this.$("#organization-control").algolia_autocomplete({hint: false}, [{
        ...
        for (let key of Array.from(SCHOOL_NCES_KEYS)) {
      ...
      return this.$("#district-control").algolia_autocomplete({hint: false}, [{
        ...
        this.state.set({showUsaStateDropdown: true});
        this.state.set({stateValue: suggestion.state});
        for (let key of Array.from(DISTRICT_NCES_KEYS)) {
          this.$('input[name="nces_' + key + '"]').val(suggestion[key]);
        }
        return this.onChangeForm();
      });
    ...
    onTrialRequestSubmit() {
      ...
      .then(() => {
        ...
        trialRequestIntercomData.educationLevel_elementary = _.contains(this.trialRequest.attributes.properties.educationLevel, "Elementary");
        trialRequestIntercomData.educationLevel_middle = _.contains(this.trialRequest.attributes.properties.educationLevel, "Middle");
        trialRequestIntercomData.educationLevel_high = _.contains(this.trialRequest.attributes.properties.educationLevel, "High");
        trialRequestIntercomData.educationLevel_college = _.contains(this.trialRequest.attributes.properties.educationLevel, "College+");
        return application.tracker.updateTrialRequestData(trialRequestIntercomData);
      }).then(() => {
    ...
    onClickGPlusSignupButton() {
      const btn = this.$('#gplus-signup-btn');
            ...
            success: () => {
              ...
              return application.gplusHandler.loadPerson({
                success: gplusAttrs => {
                  ...
                  return existingUser.fetchGPlusUser(this.gplusAttrs.gplusID, {
                    error: (user, jqxhr) => {
                      if (jqxhr.status === 404) {
    ...
    onGPlusConnected() {
      ...
      for (let field of ['email', 'firstName', 'lastName']) {
