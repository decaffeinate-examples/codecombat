...
module.exports = (RestrictedToTeachersView = (function() {
  RestrictedToTeachersView = class RestrictedToTeachersView extends RootView {
    static initClass() {
      ...
      this.prototype.template = require('templates/teachers/restricted-to-teachers-view');
    }

    initialize() {
      __guard__(me.getClientCreatorPermissions(), x => x.then(() => (typeof this.render === 'function' ? this.render() : undefined)));
      return (window.tracker != null ? window.tracker.trackEvent('Restricted To Teachers Loaded', {category: 'Students'}, ['Mixpanel']) : undefined);
    }
  };
