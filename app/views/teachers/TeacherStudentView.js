...
const fullPageTemplate = require('templates/teachers/teacher-student-view-full');
const viewTemplate = require('templates/teachers/teacher-student-view');
...
module.exports = (TeacherStudentView = (function() {
  TeacherStudentView = class TeacherStudentView extends RootView {
    ...
    getTitle() { return (this.user != null ? this.user.broadName() : undefined); }
    ...
    initialize(options, classroomID, studentID) {
      ...
      this.supermodel.trackRequest(this.levels.fetchForClassroom(classroomID, {data: {project: 'name,original,i18n,primerLanguage,thangs.id,thangs.components.config.programmableMethods.plan.solutions,thangs.components.config.programmableMethods.plan.context'}}));
    ...
    afterRender() {
      ...
      for (let oldEditor of Array.from(this.aceEditors != null ? this.aceEditors : [])) { oldEditor.destroy(); }
      this.aceEditors = [];
      const {
        aceEditors
      } = this;
      ...
      return (() => {
        ...
        // Normal level
          if (session.get('creator') === this.studentID) {var name;
            ...
            // Arena level
            result.push(this.levelStudentCodeMap[name = session.get('level').original] != null ? this.levelStudentCodeMap[name] : (this.levelStudentCodeMap[name] = __guard__(__guard__(session.get('code'), x4 => x4['hero-placeholder-1']), x3 => x3['plan'])));
          }
        }
        return result;
      })();
    ...
    calculateStandardDev() {
      ...
      for (let levelDatum of Array.from(this.levelData)) {
        levelDataByLevel[levelDatum.levelID] = levelDatum;
      }
      this.courseComparisonMap = [];
      return (() => {
        const result = [];
        for (let versionedCourse of Array.from(this.classroom.getSortedCourses() || [])) {
          ...
          for (let member of Array.from(this.classroom.get('members'))) {
            let number = 0;
            let memberPlayed = 0; // number of levels a member has played that this student has also played
            for (let versionedLevel of Array.from(versionedCourse.levels)) {
              var left;
          ...
          let perf = -(studentCourseTotal - mean) / StandardDev;
    ...
    drawBarGraph() {
      ...
      return (() => {
        ...
        // this does all of the courses, logic for whether student was assigned is in corresponding jade file
          ...
          for (let level of Array.from(this.levelData)) {
          ...
          for (let versionedLevel of Array.from(versionedCourse.levels)) {
            ...
            if ((session != null ? session.get('creator') : undefined) === this.studentID) {
              if (__guard__(session.get('state'), x => x.complete)) {
                result.push(this.levelProgressMap[versionedLevel.original] = 'complete');
              } else {
