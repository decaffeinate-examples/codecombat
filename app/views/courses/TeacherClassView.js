...
const viewContentTemplate = require('templates/courses/teacher-class-view');
const viewContentTemplateWithLayout = require('templates/courses/teacher-class-view-full');
  ...
  TeacherClassView = class TeacherClassView extends RootView {
    ...
    initialize(options, classroomID) {
      ...
      if (window.tracker != null) {
        window.tracker.trackEvent('Teachers Class Loaded', {category: 'Teachers', classroomID: this.classroom.id}, ['Mixpanel']);
      }
    ...
    attachMediatorEvents() {
      ...
      this.listenTo(this.classroom, 'sync change update', function() {
        ...
        for (let course of Array.from(this.sortedCourses)) { this.availableCourseMap[course._id] = true; }
      ...
      this.listenTo(this.students, 'sync change update add remove reset', function() {
        ...
        for (let student of Array.from(this.students.models)) {
      ...
      this.listenTo(this, 'locked-level-select:change', function({ selectedLevel }) {
        return this.setSelectedCourseLockedLevel(selectedLevel);
      });
    ...
    setSelectedCourseLockedLevel(level) {
      if (!me.showCourseProgressControl()) { return; }
    ...
    onLoaded() {
      ...
      this.latestReleasedCourses = utils.sortCoursesByAcronyms(this.latestReleasedCourses);
    ...
    calculateProgressAndLevelsAux() {
      ...
      for (let student of Array.from(this.students.models)) {
        ...
        const studentsStub = new Users([ student ]);
    ...
    getCourseAssessmentPairs() {
      ...
      for (let course of Array.from(this.courses.models)) {
        ...
        const fullLevels = _.filter(this.levels.models, l => (needle = l.get('original'), Array.from(_.map(assessmentLevels, l2=> l2.get('original'))).includes(needle)));
    ...
    onClickNavTabLink(e) {
      ...
      if (hash !== window.location.hash) {
        ...
        if (window.tracker != null) {
          window.tracker.trackEvent('Teachers Class Switch Tab', { category: 'Teachers', classroomID: this.classroom.id, tab, label: tab }, ['Mixpanel']);
        }
      }
    ...
    onClickCopyCodeButton() {
      if (window.tracker != null) {
        window.tracker.trackEvent('Teachers Class Copy Class Code', {category: 'Teachers', classroomID: this.classroom.id, classCode: this.state.get('classCode')}, ['Mixpanel']);
      }
    ...
    onClickCopyURLButton() {
      if (window.tracker != null) {
        window.tracker.trackEvent('Teachers Class Copy Class URL', {category: 'Teachers', classroomID: this.classroom.id, url: this.state.get('joinURL')}, ['Mixpanel']);
      }
    ...
    onClickUnarchive() {
      ...
      if (window.tracker != null) {
        window.tracker.trackEvent('Teachers Class Unarchive', {category: 'Teachers', classroomID: this.classroom.id}, ['Mixpanel']);
      }
    ...
    onClickEditClassroom(e) {
      if (me.id !== this.classroom.get('ownerID')) { return; } // May be viewing page as admin
      if (window.tracker != null) {
        window.tracker.trackEvent('Teachers Class Edit Class Started', {category: 'Teachers', classroomID: this.classroom.id}, ['Mixpanel']);
      }
    ...
    onClickEditStudentLink(e) {
      if (me.id !== this.classroom.get('ownerID')) { return; } // May be viewing page as admin
      if (window.tracker != null) {
        window.tracker.trackEvent('Teachers Class Students Edit', {category: 'Teachers', classroomID: this.classroom.id}, ['Mixpanel']);
      }
    ...
    onStudentRemoved(e) {
      ...
      return (window.tracker != null ? window.tracker.trackEvent('Teachers Class Students Removed', {category: 'Teachers', classroomID: this.classroom.id, userID: e.user.id}, ['Mixpanel']) : undefined);
    }

    onClickAddStudents(e) {
      if (me.id !== this.classroom.get('ownerID')) { return; } // May be viewing page as admin
      if (window.tracker != null) {
        window.tracker.trackEvent('Teachers Class Add Students', {category: 'Teachers', classroomID: this.classroom.id}, ['Mixpanel']);
      }
    ...
    onChangeLockedLevelSelect(e) {
      return this.trigger('locked-level-select:change', { selectedLevel: $(e.currentTarget).val() });
    }
    ...
    onClickEnrollStudentButton(e) {
      ...
      return window.tracker != null ? window.tracker.trackEvent($(e.currentTarget).data('event-action'), {category: 'Teachers', classroomID: this.classroom.id, userID}, ['Mixpanel']) : undefined;
    }
    ...
    onClickExportStudentProgress() {
      ...
      let c;
      if (window.tracker != null) {
        window.tracker.trackEvent('Teachers Class Export CSV', {category: 'Teachers', classroomID: this.classroom.id}, ['Mixpanel']);
      }
      let courseLabels = "";
      const courses = ((() => {
        const result = [];
      ...
      for (let student of Array.from(this.students.models)) {
        ...
        for (trimCourse of Array.from(this.classroom.getSortedCourses())) {
          ...
          const instance = this.courseInstances.findWhere({ courseID: course.id, classroomID: this.classroom.id });
          if (instance && instance.hasMember(student)) {
            for (trimLevel of Array.from(trimCourse.levels)) {
              ...
              const progress = this.state.get('progressData').get({ classroom: this.classroom, course, level, user: student });
        ...
        let levels = 0;
        let playtime = 0;
        for (let session of Array.from(this.classroom.sessions.models)) {
    ...
    onClickAssignStudentButton(e) {
      ...
      return window.tracker != null ? window.tracker.trackEvent('Teachers Class Students Assign Selected', {category: 'Teachers', classroomID: this.classroom.id, courseID, userID}, ['Mixpanel']) : undefined;
    }
    ...
    onClickBulkAssign() {
      ...
      return window.tracker != null ? window.tracker.trackEvent('Teachers Class Students Assign Selected', {category: 'Teachers', classroomID: this.classroom.id, courseID}, ['Mixpanel']) : undefined;
    }
    ...
    onClickBulkRemoveCourse() {
      ...
      return window.tracker != null ? window.tracker.trackEvent('Teachers Class Students Remove-Course Selected', {category: 'Teachers', classroomID: this.classroom.id, courseID}, ['Mixpanel']) : undefined;
    }
      ...
      .then(() => {
        ...
        for (prepaid of Array.from(availablePrepaids)) {
          if (Math.min(_.size(unenrolledStudents), prepaid.openSpots()) > 0) {
            for (let i = 0, end = Math.min(_.size(unenrolledStudents), prepaid.openSpots()), asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
              const user = unenrolledStudents.shift();
    ...
    calculateClassStats() {
      ...
      const language = __guard__(this.classroom.get('aceConfig'), x => x.language);
      for (let level of Array.from(this.levels.models)) {
