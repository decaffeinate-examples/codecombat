...
const ProjectGalleryComponent = Vue.extend({
  ...
  computed: {
    levelName() { return this.level && utils.i18n(this.level, 'name'); },
  ...
  created() {
    return Promise.all([
      ...
      me.isSchoolAdminOf({ classroomId: this.courseInstance.classroomID }).then(res => { return this.amSchoolAdministratorOfGallery = res; });
      me.isTeacherOf({ classroomId: this.courseInstance.classroomID }).then(res => { return this.amTeacherOfGallery = res; });
