...
const template = require('templates/account/subscription-view');
...
class EmailValidator {
  ...
  validateEmails(emails, render) {
    ...
    this.validEmails = ((() => {
      const result = [];
      for (let email of Array.from(emails)) {         if (emailRegex.test(email.trim().toLowerCase())) {
          result.push(email);
        }
      }
      return result;
  ...
  unsubscribe(message, render) {
    const removeSub = () => {
      ...
      if (payPalInfo != null ? payPalInfo.billingAgreementID : undefined) {
        return api.users.cancelBillingAgreement({userID: me.id, billingAgreementID: (payPalInfo != null ? payPalInfo.billingAgreementID : undefined)})
        .then(response => {
          if (window.tracker != null) {
            window.tracker.trackEvent('Unsubscribe End', {message});
          }
          return document.location.reload();
      }).catch(jqxhr => {
    ...
    if (message) {
      return $.post('/contact', {message, subject: 'Cancellation'}, response => removeSub());
    } else {
      return removeSub();
    }
  }
      ...
      if (stripeInfo.sponsorID) {
        ...
        options.success = info => {
          ...
          if (sub = info.subscription) {
            ...
            console.error(`Could not find personal subscription ${__guard__(me.get('stripe'), x3 => x3.customerID)} ${__guard__(me.get('stripe'), x4 => x4.subscriptionID)}`);
          }
          delete this.state;
          return render();
        };
        ...
        payments.once('sync', function() {
          this.monthsSubscribed = ((() => {
            const result = [];
            for (let x of Array.from(payments.models)) {               if (!x.get('productID')) {
                result.push(x);
              }
            }
            return result;
...
class RecipientSubs {
  ...
  update(render) {
    ...
    const onSubRecipientsSuccess = recipientsMap => {
      ...
      let count = 0;
      for (let userID in this.recipients) {
