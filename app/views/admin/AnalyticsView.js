...
const template = require('templates/admin/analytics');
  ...
  AnalyticsView = class AnalyticsView extends RootView {
    ...
    loadData() {
      this.supermodel.addRequestResource({
        ...
        success: data => {
          ...
          const groupMap = {};
          const dayGroupMap = {};
          for (let activeClass of Array.from(data)) {
      ...
      this.supermodel.addRequestResource({
        ...
        success: data => {
          ...
          const eventMap = {};
          for (let entry of Array.from(this.activeUsers)) {
            ({
              day
            } = entry);
      ...
      this.supermodel.addRequestResource({
        ...
        success: data => {
          ...
          let dailyGroup, day, group, i, monthlyGroup;
          const revenueGroupFromPayment = function(payment) {
          ...
          for (let payment of Array.from(data)) {
          ...
          const lifetimeDurationMonths = 8; // Needs to be an integer
          ...
          for (day in dayGroupCountMap) {
            ...
            for (group in serviceCarryForwardMap) {
              for (let carryData of Array.from(serviceCarryForwardMap[group])) {
          ...
          this.monthMrrMap = {};
          for (let revenue of Array.from(this.revenue)) {
            ...
            for (i = 0; i < this.revenueGroups.length; i++) {
              ...
              } else if (['DRR usa lifetime', 'DRR intl lifetime'].includes(group)) {
      ...
      this.supermodel.addRequestResource({
        ...
        method: 'POST',
        data: {minCount: this.minSchoolCount},
        success: schoolCounts => {
          this.schoolCounts = schoolCounts;
          if (this.schoolCounts != null) {
            this.schoolCounts.sort(function(a, b) {
            if (a.count > b.count) { return -1; }
            if (a.count === b.count) { return 0; }
            return 1;
          });
          }
          return (typeof this.renderSelectors === 'function' ? this.renderSelectors('#school-counts') : undefined);
        }
      }, 0).load();
      ...
      this.supermodel.addRequestResource({
        ...
        success: schoolSales => {
          this.schoolSales = schoolSales;
          if (this.schoolSales != null) {
            this.schoolSales.sort(function(a, b) {
            if (a.created > b.created) { return -1; }
            if (a.created === b.created) { return 0; }
            return 1;
          });
          }
          return (typeof this.renderSelectors === 'function' ? this.renderSelectors('.school-sales') : undefined);
        }
      }, 0).load();
      ...
      this.supermodel.addRequestResource({
        ...
        success: prepaids => {
          ...
          for (let prepaid of Array.from(prepaids)) {
    ...
    onCourseInstancesSync(data) {
      ...
      const createCourseDistributions = numDays => {
        ...
        for (let courseInstance of Array.from(data.courseInstances)) {
          ...
          for (let studentID of Array.from(courseInstance.members)) {
        ...
        for (user of Array.from(data.students)) {
          ...
          if (prepaidID = user.coursePrepaid != null ? user.coursePrepaid._id : undefined) {
            studentPaidStatusMap[user._id] = 'paid';
            if (prepaidUserMap[prepaidID] == null) { prepaidUserMap[prepaidID] = []; }
            prepaidUserMap[prepaidID].push(user._id);
          }
        }

        // Find trial students
        for (let prepaid of Array.from(data.prepaids)) {
          if (!prepaidUserMap[prepaid._id]) { continue; }
          if (prepaid.properties != null ? prepaid.properties.trialRequestID : undefined) {
            for (let userID of Array.from(prepaidUserMap[prepaid._id])) {
        ...
        for (let teacher in teacherStudentsMap) {
          const students = teacherStudentsMap[teacher];
          for (let student of Array.from(students)) {
        ...
        const updateCourseTotalsMap = (courseTotalsMap, furthestCourseMap, paidStatusMap, columnSuffix) => {
          return (() => {
            ...
            for (user in furthestCourseMap) {
              ...
              const columnName = (() => { switch (paidStatusMap[user]) {
    ...
    updateActiveClassesChartData() {
      ...
      const groupDayMap = {};
      for (let entry of Array.from(this.activeClasses)) {
    ...
    updateRevenueChartData() {
      ...
      const groupDayMap = {};
      for (let entry of Array.from(this.revenue)) {
        for (let i = 0; i < entry.groups.length; i++) {
          count = entry.groups[i];
