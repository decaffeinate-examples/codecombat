...
module.exports = (AdminClassroomsProgressView = (function() {
  AdminClassroomsProgressView = class AdminClassroomsProgressView extends RootView {
    static initClass() {
      ...
      this.prototype.template = require('templates/admin/admin-classrooms-progress');
    ...
    initialize() {
      ...
      this.courseColorMap = Object.keys(utils.courseAcronyms).reduce((m, c, i) => {
      ...
      .then(results => {
        ...
        for (let prepaid of Array.from(prepaids)) {
          if (!adminMap[prepaid.creator.toString()]) {
            for (let student of Array.from(prepaid.redeemers || [])) { studentPrepaidMap[student.userID.toString()] = true; }
          }
        }
        ...
        let levels = ((() => {
          const result1 = [];
          for (let original in levelOriginalStringsMap) {
        ...
        let studentIds = [];
        for (classroom of Array.from(this.classrooms)) {
          for (let studentId of Array.from(classroom.members)) {
        ...
        var fetchLevelSessions = (i, results) => {
          ...
          .then(resultsMatrix => {
            for (let newResults of Array.from(resultsMatrix)) {
          ...
          for (var classroomId in classroomLicenseCourseLevelMap) { //when classroomId is '573ac4b48edc9c1f009cd6be'
            ...
            for (var licenseId in licensesCourseLevelMap) {
              ...
              for (courseId in courseLevelMap) {
                ...
                for (let levelOriginal in levelMap) {
    ...
    addAvailableCourseMissingLevels(classroomId, classroomLicenseFurthestLevelMap, courseId, courseLastLevelIndexes, courseLevelMap, latestCourseLevelOriginals, latestOrderedLevelOriginals, levels, licenseId) {
      ...
      return (() => {
        ...
        for (let i = 0; i < latestLevelsToAdd.length; i++) { //when @latestCourseMap[courseId].slug is 'computer-science-4'
          ...
          result1.push((() => {
            const result2 = [];
            for (let courseLastLevelIndexData of Array.from(courseLastLevelIndexes)) {
    ...
    getClassroomActivity(classrooms, latestCourseMap, userLatestActivityMap, userLicensesMap, userLevelOriginalCompleteMap, userLevelOriginalStartedMap) {
      ...
      for (let classroom of Array.from(classrooms)) { //when classroom._id is '573ac4b48edc9c1f009cd6be'
        for (let license of Array.from(userLicensesMap[classroom.ownerID])) {
          ...
          for (userId in userLevelOriginalCompleteMap) {
            ...
            if (licensedMembers.indexOf(userId) >= 0) {
              ...
              for (let levelOriginal in levelOriginalCompleteMap) {
          ...
          for (course of Array.from(utils.sortCourses(classroom.courses))) {
            if (latestCourseMap[course._id]) {
              ...
              const levelCompleteDates = ((() => {
                const result = [];
                for (let levelId in levelCompleteDateMap) {
              ...
              levelCompleteDates.reduce((sum, currentDate) => {
                ...
                for (let target of Array.from(this.targetPercentCompleted)) {
    ...
    getLatestLevels(campaigns, courses) {
      ...
      for (let course of Array.from(courses)) {
        ...
        for (let levelOriginal in campaign.levels) {
    ...
    getUserActivity(levelSessions, licenses, latestOrderedLevelOriginals) {
      ...
      for (let levelSession of Array.from(levelSessions)) {
      ...
      for (let license of Array.from(licenses)) {
    ...
    sortClassroomProgress(classroomProgress) {
      ...
      for (let progress of Array.from(classroomProgress)) {
