...
module.exports = (SchoolCountsView = (function() {
  SchoolCountsView = class SchoolCountsView extends RootView {
    static initClass() {
      ...
      this.prototype.template = require('templates/admin/school-counts');
      this.prototype.state = '';
    }

    initialize() {
      if (!me.isAdmin()) { return super.initialize(); }
      this.batchSize = utils.getQueryVariable('batchsize', 50000);
      this.loadData();
    ...
    loadData() {
      var fetchBatch = (baseUrl, results, beforeId) => {
        ...
        return new Promise(resolve => setTimeout(resolve.bind(null, Promise.resolve($.get(url))), 100))
        .then(batchResults => {
      ...
      .then((...args) => {
        ...
        for (let courseInstance of Array.from(courseInstances)) {
        ...
        for (let classroom of Array.from(classrooms)) {
        ...
        for (let studentId of Array.from(studentNonHocMap)) { delete studentNonHocMap[studentId]; } // Don't need these anymore
        ...
        for (let trialRequest of Array.from(trialRequests)) {
