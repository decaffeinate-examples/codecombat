...
module.exports = (AdminClassroomContentView = (function() {
  AdminClassroomContentView = class AdminClassroomContentView extends RootView {
    static initClass() {
      ...
      this.prototype.template = require('templates/admin/admin-classroom-content');
    }
    ...
    fetchData(userIds) {
      ...
      const userIdMap = {};
      for (let id of Array.from((userIds || []))) { userIdMap[id] = true; }
      var getMoreLevelSessions = (courseIDs, startOffset, endOffset) => {
        ...
        .then(results => {
          ...
          for (let index = 0; index < results.length; index++) {
            ...
            for (let levelPlaytime of Array.from(levelPlaytimes)) {
            ...
            for (let session of Array.from(levelSessions)) {
          ...
          if ((startOffset <= this.maxDays) && (courseIDs.length > 0)) {
            ...
            this.courseSeconds = ((() => {
              const result1 = [];
              for (let courseSlug in courseSecondsMap) {
      ...
      return getMoreLevelSessions(((() => {
        const result = [];
        for (let key in utils.courseIDs) {
