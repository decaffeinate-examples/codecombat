...
const template = require('templates/clans/clans');
...
module.exports = (ClansView = (function() {
  ClansView = class ClansView extends RootView {
    ...
    loadData() {
      ...
      this.publicClans = new CocoCollection([], { url: '/db/clan/-/public', model: Clan, comparator: sortClanList });
    ...
    refreshNames(clans) {
      ...
      const options = {
        ...
        success: (models, response, options) => {
          for (let userID in models) { this.idNameMap[userID] = models[userID].name; }
          return (typeof this.render === 'function' ? this.render() : undefined);
        }
      };
