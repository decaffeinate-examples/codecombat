...
module.exports = (LadderTabView = (function() {
  LadderTabView = class LadderTabView extends CocoView {
    ...
    static initClass() {
      ...
      this.prototype.template = require('templates/play/ladder/ladder-tab-view');
    ...
    render() {
      ...
      return this.$el.find('.histogram-display').each((i, el) => {
        ...
        return $.when(
          ...
          (url = `/db/level/${level}/histogram_data?team=${team.name.toLowerCase()}&levelSlug=${this.level.get('slug')}`),
...
module.exports.LeaderboardData = (LeaderboardData = (LeaderboardData = class LeaderboardData extends CocoClass {
  ...
  */
  ...
  fetch() {
    ...
    this.topPlayers = new LeaderboardCollection(this.level, this.collectionParameters({order: -1, scoreOffset: HIGHEST_SCORE, limit: this.limit}));
    const promises = [];
    ...
    if (this.session) {
      ...
      if (score) {
        ...
        const level = `${this.level.get('original')}.${this.level.get('version').major}`;
        const success = myRank => {
          this.myRank = myRank;
        };
        let loadURL = `/db/level/${level}/leaderboard_rank?scoreOffset=${score}&team=${this.team}&levelSlug=${this.level.get('slug')}`;
