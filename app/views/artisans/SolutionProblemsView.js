...
const template = require('templates/artisans/solution-problems-view');
...
module.exports = (SolutionProblemsView = (function() {
  ...
  SolutionProblemsView = class SolutionProblemsView extends RootView {
    ...
    onLevelsLoaded(lvlCollection) {
      for (let level of Array.from(lvlCollection.models)) {
    ...
    onAllLevelsLoaded() {
      for (let levelSlug in this.loadedLevels) {
    ...
    findSimulationProblems(solution) {
      const problems = [];
      for (let req of Array.from(simulationRequirements)) {
