...
const template = require('templates/editor/level/scripts_tab');
...
class EventPropsNode extends TreemaNode.nodeMap.string {
  ...
  buildValueForEditing(valEl, data) {
    ...
    for (let key in (channelSchema != null ? channelSchema.properties : undefined)) { const val = (channelSchema != null ? channelSchema.properties : undefined)[key]; autocompleteValues.push(key); }
...
class ChannelNode extends TreemaNode.nodeMap.string {
  buildValueForEditing(valEl, data) {
    ...
    const autocompleteValues = ((() => {
      const result = [];
      for (let key in Backbone.Mediator.channelSchemas) {
