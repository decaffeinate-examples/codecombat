...
const template = require('templates/editor/level/system/add');
const availableSystemTemplate = require('templates/editor/level/system/available_system');
...
module.exports = (AddLevelSystemModal = (function() {
  AddLevelSystemModal = class AddLevelSystemModal extends ModalView {
    ...
    renderAvailableSystems() {
      ...
      return (() => {
        const result = [];
        for (let system of Array.from(systems)) {
    ...
    onAddSystem(e) {
      ...
      for (let s of Array.from(toAdd.concat([system]))) {
        var left, left1;
        const levelSystem = {
