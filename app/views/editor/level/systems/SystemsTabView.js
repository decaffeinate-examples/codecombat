...
const template = require('templates/editor/level/systems-tab-view');
...
module.exports = (SystemsTabView = (function() {
  SystemsTabView = class SystemsTabView extends CocoView {
    ...
    constructor(options) {
      ...
      super(options);
      for (let system of Array.from(this.buildDefaultSystems())) {
    ...
    getSortedByName(systems) {
      ...
      for (let sys of Array.from(systemModels)) { systemModelMap[sys.get('original')] = sys.get('name'); }
    ...
    editLevelSystem(e) {
      return this.levelSystemEditView = this.insertSubView(new LevelSystemEditView({original: e.original, majorVersion: e.majorVersion, supermodel: this.supermodel}));
    }
