...
const template = require('templates/editor/level/level-edit-view');
  ...
  LevelEditView = class LevelEditView extends RootView {
    ...
    onLoaded() {
      _.defer(() => {
        ...
        this.render();
        return this.timerIntervalID = setInterval(this.incrementBuildTime, 1000);
      });
      const campaignCourseMap = {};
      for (let course of Array.from(this.courses.models)) { campaignCourseMap[course.get('campaignID')] = course.id; }
    ...
    afterRender() {
      ...
      this.insertSubView(new ThangsTabView({world: this.world, supermodel: this.supermodel, level: this.level}));
    ...
    onPopulateI18N() {
      ...
      const levelComponentMap = _(currentView.supermodel.getModels(LevelComponent))
