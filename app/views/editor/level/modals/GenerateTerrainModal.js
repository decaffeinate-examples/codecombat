...
const template = require('templates/editor/level/modal/generate-terrain-modal');
...
module.exports = (GenerateTerrainModal = (function() {
  GenerateTerrainModal = class GenerateTerrainModal extends ModalView {
    ...
    generateDecorations(preset, presetSize){
      return (() => {
        ...
        for (var name in preset.decorations) {
          ...
          result.push((() => {
            const result1 = [];
            for (let num of Array.from(_.range(presetSize.sizeFactor * _.random(decoration.num[0], decoration.num[1])))) {
              var rect;
    ...
    addThang(thang) {
      ...
      for (let existingThang of Array.from(this.thangs)) {
    ...
    addRect(rect) {
      ...
      for (let existingRect of Array.from(this.rects)) {
