...
const template = require('templates/editor/level/modal/save-branch-modal');
  ...
  SaveBranchModal = class SaveBranchModal extends ModalView {
    ...
    initialize({ components, systems }) {
      ...
      .then(() => {
        ...
        const fetches = [];
        for (let branch of Array.from(this.branches.models)) {
          for (let patch of Array.from(branch.get('patches') || [])) {
        ...
        for (let branch of Array.from(this.branches.models)) {
          ...
          for (let patch of Array.from(branch.get('patches') || [])) {
            ...
            const model = allModels.get(patch.target.id).clone(false);
            model.markToRevert();
    ...
    afterRender() {
      ...
      for (changeEl of Array.from(changeEls)) {
        ...
        const component = this.componentsWithChanges.find(c => c.id === componentId);
      ...
      return (() => {
        ...
        for (changeEl of Array.from(changeEls)) {
          ...
          const system = this.systemsWithChanges.find(c => c.id === systemId);
    ...
    renderSelectedBranch() {
      ...
      if (this.selectedBranchDeltaViews) { for (let view of Array.from(this.selectedBranchDeltaViews)) { this.removeSubView(view); } }
      ...
      for (changeEl of Array.from(changeEls)) {
        const componentId = $(changeEl).data('component-id');
        var component = this.selectedBranch.components.get(componentId);
        const targetComponent = this.components.find(c => (c.get('original') === component.get('original')) && c.get('version').isLatestMajor);
        preBranchSave = component.clone();
        preBranchSave.markToRevert();
        const componentDiff = targetComponent.clone();
      ...
      return (() => {
        const result = [];
        for (changeEl of Array.from(changeEls)) {
          const systemId = $(changeEl).data('system-id');
          var system = this.selectedBranch.systems.get(systemId);
          const targetSystem = this.systems.find(c => (c.get('original') === system.get('original')) && c.get('version').isLatestMajor);
          preBranchSave = system.clone();
          preBranchSave.markToRevert();
          const systemDiff = targetSystem.clone();
    ...
    saveBranch(e, {deleteSavedChanges}) {
      ...
      for (let component of Array.from(selectedComponents)) {
      ...
      for (let system of Array.from(selectedSystems)) {
      ...
      .then(() => {
        if (deleteSavedChanges) {
          for (let model of Array.from(toRevert)) { model.revert(); }
        }
        return this.hide();
    }).catch(e => {
