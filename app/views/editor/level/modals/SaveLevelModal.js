...
const template = require('templates/editor/level/save-level-modal');
...
module.exports = (SaveLevelModal = (function() {
  SaveLevelModal = class SaveLevelModal extends SaveVersionModal {
    ...
    afterRender() {
      ...
      models = ((() => {
        const result = [];
        for (let m of Array.from(models)) {           if (m.hasWriteAccess()) {
            result.push(m);
          }
        }
        return result;
    ...
    commitLevel(e) {
      ...
      for (form of Array.from(this.$el.find('form'))) {
        ...
        const fields = {};
        for (let field of Array.from($(form).serializeArray())) {
