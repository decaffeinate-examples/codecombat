...
const template = require('templates/editor/level/components_tab');
...
module.exports = (ComponentsTabView = (function() {
  ComponentsTabView = class ComponentsTabView extends CocoView {
    ...
    refreshLevelThangsTreema(thangsData) {
      ...
      for (let thang of Array.from(thangsData)) {
        var left;
        const componentMap = {};
        ...
        for (component of Array.from(_.values(componentMap))) {
          var name;
          const haveThisComponent = (presentComponents[name = component.original + '.' + (component.majorVersion != null ? component.majorVersion : 0)] != null ? presentComponents[name] : (presentComponents[name] = []));
      ...
      for (let comp of Array.from(componentModels)) { componentModelMap[comp.get('original')] = comp; }
      let components = ((() => {
        const result = [];
      ...
      components = components.concat(((() => {
        const result1 = [];
        for (let c of Array.from(componentModels)) {           if (!this.presentComponents[c.get('original') + '.' + c.get('version').major]) {
    ...
    editLevelComponent(e) {
      return this.levelComponentEditView = this.insertSubView(new LevelComponentEditView({original: e.original, majorVersion: e.majorVersion, supermodel: this.supermodel}));
    }
