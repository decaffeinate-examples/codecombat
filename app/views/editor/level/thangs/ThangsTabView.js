...
const thangs_template = require('templates/editor/level/thangs-tab-view');
  ...
  ThangsTabView = class ThangsTabView extends CocoView {
    ...
    constructor(options) {
      ...
      this.listenToOnce(this.componentCollection, 'sync', function() {
        return (() => {
          const result = [];
          for (let component of Array.from(this.componentCollection.models)) {
    ...
    getRenderData(context) {
      ...
      const groups = [];
      for (let groupName of Array.from(Object.keys(groupMap).sort())) {
    ...
    onSpriteDragged(e) {
      ...
      if (xDiff || yDiff) {
        for (let singleSelected of Array.from(selected.slice(0, selected.length - 1))) {
          const newPos = {
      ...
      for (let sidebarWidth of Array.from(sidebarWidths)) { w -= sidebarWidth; }
    ...
    onSpriteMouseUp(e) {
      ...
      for (let singleSelected of Array.from(selected)) {
        var left;
        const {
          pos
        ...
        const path = `${this.pathForThang(thang)}/components/original=${LevelComponent.PhysicalID}`;
    ...
    createAddThang() {
      ...
      const components = [];
      for (let raw of Array.from(mockThang.components)) {
    ...
    deleteSelectedExtantThang(e) {
      ...
      for (let singleSelected of Array.from(selected)) {
        ...
        Thang.resetThangIDs();
      }  // TODO: find some way to do this when we delete from treema, too
      return this.gameUIState.set('selected', []);
    }
    ...
    groupThangs(thangs) {
      ...
      for (let index = 0; index < thangs.length; index++) {
        ...
        let obj = grouped;
        for (let key of Array.from(path)) {
    ...
    populateFoldersForThang(thang) {
      ...
      return (() => {
        const result = [];
        for (let segment of Array.from(thangFolder)) {
          if (prefix) { prefix += '/'; }
          prefix += segment;
    ...
    rotateSelectedThangTo(radians) {
      return (() => {
        const result = [];
        for (let singleSelected of Array.from(this.gameUIState.get('selected'))) {
          var selectedThang = singleSelected.thang;
          result.push(this.modifySelectedThangComponentConfig(selectedThang, LevelComponent.PhysicalID, component => {
    ...
    toggleSelectedThangCollision() {
      return (() => {
        ...
        for (let singleSelected of Array.from(this.gameUIState.get('selected'))) {
          var selectedThang = singleSelected.thang;
          result.push(this.modifySelectedThangComponentConfig(selectedThang, LevelComponent.CollidesID, component => {
...
//      el.prepend($("<img class='img-circle' src='#{thangType.getPortraitURL()}' />"))
  ...
  nameToThangType(name) {
    if (!ThangsFolderNode.nameToThangTypeMap) {
      ...
      const map = {};
      for (let thangType of Array.from(thangTypes)) { map[thangType.get('name')] = thangType; }
