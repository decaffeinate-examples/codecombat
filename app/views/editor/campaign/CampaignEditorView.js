...
module.exports = (CampaignEditorView = (function() {
  CampaignEditorView = class CampaignEditorView extends RootView {
    static initClass() {
      ...
      this.prototype.template = require('templates/editor/campaign/campaign-editor-view');
    ...
    onLeaveMessage() {
      for (let model of Array.from(this.toSave.models)) {
    ...
    loadThangTypeNames() {
      ...
      let originals = [];
      for (let level of Array.from(_.values(this.campaign.get('levels')))) {
      ...
      return (() => {
        const result = [];
        for (let original of Array.from(originals)) {
    ...
    updateCampaignLevels() {
      ...
      for (let levelIndex = 0; levelIndex < this.levels.models.length; levelIndex++) {
        ...
        if ((this.campaign.get('type') === 'hero') || this.campaign.get('isOzaria')) { campaignLevel.campaign = this.campaign.get('slug'); }
      ...
      return (() => {
        ...
        for (level of Array.from(_.values(campaignLevels))) {
          ...
          const model = this.levels.findWhere({original: level.original});
          ...
          for (let key of Array.from(propsToPropagate)) {
    ...
    formatRewards(level) {
      ...
      const rewards = [];
      for (let achievement of Array.from(achievements)) {
        const object = achievement.get('rewards');
        for (let rewardType in object) {
          const rewardArray = object[rewardType];
          for (let reward of Array.from(rewardArray)) {
            var thangType;
    ...
    propagateCampaignIndexes() {
      ...
      return (() => {
        const result = [];
        for (let levelOriginal in campaignLevels) {
          const campaignLevel = campaignLevels[levelOriginal];
          if (this.campaign.get('type') === 'course') {
    ...
    onAnalyticsModalClosed(options) {
      if ((options.targetLevelSlug != null) && (__guard__(this.treema.childrenTreemas != null ? this.treema.childrenTreemas.levels : undefined, x => x.childrenTreemas) != null)) {
        return (() => {
          const result = [];
          for (let original in this.treema.childrenTreemas.levels.childrenTreemas) {
...
class LevelsNode extends TreemaObjectNode {
  ...
  childSource(req, res) {
    ...
    return s.once('sync', collection => {
      for (let level of Array.from(collection.models)) {
...
class NextLevelNode extends LevelNode {
  populateData() {
    if (this.data.name != null) { return; }
    const data = _.pick(LevelsNode.levels[this.keyForParent].attributes, ['original', 'name', 'slug', 'type']);
    return _.extend(this.data, data);
  }
}
...
class CampaignsNode extends TreemaObjectNode {
  ...
  childSource(req, res) {
    ...
    return s.once('sync', function(collection) {
      for (let campaign of Array.from(collection.models)) { CampaignsNode.campaigns[campaign.id] = campaign; }
...
class RewardsNode extends TreemaArrayNode {
  buildValueForDisplay(valEl, data) {
    super.buildValueForDisplay(valEl, data);
    let achievements = window.currentView.achievements.where({related: this.parent.data.original});
