...
const template = require('templates/editor/campaign/campaign-analytics-modal');
...
module.exports = (CampaignAnalyticsModal = (function() {
  CampaignAnalyticsModal = class CampaignAnalyticsModal extends ModalView {
    ...
    addCompletionLineGraphs() {
      ...
      return (() => {
        const result = [];
        for (let level of Array.from(this.campaignCompletions.levels)) {
          var day;
          const days = [];
          ...
          const data = [];
          for (let i = 0, end = days.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
            data.push({
              x: i,
              y: days[i].rate,
              c: days[i].count
        ...
        for (let level of Array.from(this.campaignCompletions.levels)) {
          if (levelAverages[level.level]) {
            if (levelAverages[level.level].length > 0) {
              const total = _.reduce(levelAverages[level.level], ((sum, num) => sum + num));
