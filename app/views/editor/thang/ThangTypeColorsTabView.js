...
const template = require('templates/editor/thang/colors_tab');
...
module.exports = (ThangTypeColorsTabView = (function() {
  ThangTypeColorsTabView = class ThangTypeColorsTabView extends CocoView {
    ...
    getColorConfig() {
      ...
      const tintMap = {};
      for (let tint of Array.from(this.tintAssignments.data)) {
        tintMap[tint.name] = tint;
      }

      for (let k in this.tintedColorChoices) {
    ...
    createShapeButtons() {
      ...
      let colors = ((() => {
        const result1 = [];
        for (let s of Array.from(shapes)) {           if (s.fc != null) {
            result1.push(s.fc);
          }
        }
        return result1;
