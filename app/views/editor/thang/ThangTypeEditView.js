...
const template = require('templates/editor/thang/thang-type-edit-view');
...
const AnimateImporterWorker = require('worker-loader!./AnimateImportWorker.js');
...
module.exports = (ThangTypeEditView = (function() {
  ...
  //    @$el.find('#upload-button').prop('disabled', true)
    ...
    onNewSpriteSheet() {
      $('#spritesheets').empty();
      for (let image of Array.from(this.layerAdapter.spriteSheet._images)) {
    ...
    getLankOptions() { return {resolutionFactor: this.resolution, thang: this.mockThang, preloadSounds: false}; }
    ...
    openSaveModal() {
      const modal = new SaveVersionModal({model: this.thangType});
      this.openModalView(modal);
