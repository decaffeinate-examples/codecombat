...
class CocoModel extends Backbone.Model {
  ...
  initialize(attributes, options) {
    ...
    if (window.application != null ? window.application.testing : undefined) {
      this.fakeRequests = [];
  ...
  clone(withChanges) {
    ...
    if (this._revertAttributes && !withChanges) {
      ...
      for (let key of Array.from(_.difference(_.keys(clone.attributes), _.keys(this._revertAttributes)))) {
        clone.unset(key);
      }
    }
    return clone;
  ...
  patch(options) {
    ...
    const keys = [];
    for (let key of Array.from(_.keys(this.attributes))) {
  ...
  markToRevert() {
    if (this.type() === 'ThangType') {
      ...
      return (() => {
        const result = [];
        for (let smallProp in this.attributes) {
  ...
  isPublished() {
    let left;
    for (let permission of Array.from(((left = this.get('permissions', true)) != null ? left : []))) {
