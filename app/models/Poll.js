    ...
    applyDelta(delta) {
      ...
      for (let answerIndex in object) {
        ...
        if (_.isArray(answerChanges)) {
          ...
          for (let change of Array.from(answerChanges)) {
            var pickedChange;
            if (_.isNumber(change)) {
              pickedChange = change;
            } else {
              pickedChange = $.extend(true, {}, change);
              for (let key in pickedChange) {
