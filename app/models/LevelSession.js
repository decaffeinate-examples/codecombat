...
module.exports = (LevelSession = (function() {
  LevelSession = class LevelSession extends CocoModel {
    ...
    readyToRank() {
      ...
      const thangSpellArr = (Array.from(this.get('teamSpells')[team]).map((s) => s.split('/')));
      for (let item of Array.from(thangSpellArr)) {
        const thang = item[0];
        const spell = item[1];
    ...
    shouldAvoidCorruptData(attrs) {
      let left;
      if (me.team !== 'humans') { return false; }
      if (_.string.startsWith((left = __guard__(__guard__(((attrs != null ? attrs.code : undefined) != null ? (attrs != null ? attrs.code : undefined) : this.get('code')), x1 => x1.anya), x => x.makeBid)) != null ? left : '', 'var __interceptThis')) {
    ...
    save(attrs, options) {
      if (this.shouldAvoidCorruptData(attrs)) { return; }
    ...
    static getTopScores({level, session}) {
      ...
      if (level) {
        for (let sessionScore of Array.from(scores)) {
    ...
    inLeague(leagueId) {
      ...
      for (let league of Array.from(this.get('leagues'))) {
    ...
    saveKeyValueDb() {
      ...
      for (let key in keyValueDb) {
        ...
        if (!oldValue || (typeof(oldValue) === 'string') || (typeof(value) === 'string')) {
