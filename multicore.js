...
if (process.env.COCO_CORES) { numCPUs = parseInt(process.env.COCO_CORES); }
...
if (process.env.COCO_DEBUG_PORT != null) {
  require('./debugger').init();
}

if (cluster.isMaster) {
  for (let i = 0, end = numCPUs, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
    cluster.fork();
  }
  cluster.on('exit', function(worker, code, signal) {
    const message = `Worker ${worker.id} died!`;
    console.log(message);
    try {
  ...
  const {app, httpServer} = server.startServer();
