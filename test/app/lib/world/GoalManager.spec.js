...
describe('GoalManager', function() {
  ...
  beforeEach(() => session = factories.makeLevelSession({ state: { complete: false }}));
  ...
  it('adds new additional goals without affecting old goals', function() {
    ...
    const goalStates = gm.getGoalStates();
    ...
    return expect(gm.getRemainingGoals().length).toBe(2);
  });
  ...
  it('adds all additionalGoals for the next stage', function() {
    ...
    return expect(gm.getRemainingGoals().length).toBe(0);
  });
  ...
  it('reports that all goals are complete when additionalGoals have been completed', function() {
    ...
    expect(goalStates.additionalsaveguy.keyFrame).toBe('end');
    expect(gm.getRemainingGoals().length).toBe(0);
    return expect(gm.checkOverallStatus()).toBe('success');
  });
  ...
  it('reports that not all goals are complete when not all additionalGoals have been completed', function() {
    ...
    return expect(gm.checkOverallStatus()).not.toBe('success');
  });

  it('progresses to the next additional goal when completing them all', function() {
  ...
  it('does not add goals twice for the same stage', function() {
    const gm = new GoalManager(null, [saveGoal], {}, {
      session,
      additionalGoals
    });
